...
class FaceDetector:
    ...
    def __init__(self, 
        ...
        self.room_trackers = {}   # roomId -> MotionTracker
        self.panic_mode = False

        # Motion tracking enabled by default for fast-moving objects
        self.enable_motion_tracking = True

        print(f"[FaceDetector] Initialized with ctx_id={self.ctx_id}")
        print(f"[FaceDetector] Motion tracking enabled: {self.enable_motion_tracking}")
    ...
    def _get_room_tracker(self, room_id: str) -> MotionTracker:
        """Get or create room-specific motion tracker."""
        if room_id not in self.room_trackers:
            self.room_trackers[room_id] = MotionTracker(
                max_corners=30,
                quality_level=0.01,
                min_distance=7,
                velocity_expansion_factor=1.5,
                max_velocity_expansion=50
            )
            print(f"[FaceDetector] Created motion tracker for room {room_id}")
        return self.room_trackers[room_id]
    ...
    def cleanup_room(self, room_id: str):
        ...
        if room_id in self.room_trackers:
            del self.room_trackers[room_id]
    ...
    def process_frame(self, frame: np.ndarray, frame_id: int, 
        ...
        if self.panic_mode:
            ...
            max_confidence_this_frame = 0.0  # Track highest confidence in this frame
            ...
            if self.creator_embedding is not None and len(faces) > 0:
                ...
                for area, i, face, box in top_faces:
                    try:
                        ...
                        # Calculate distance and confidence score
                        distance = self.cosine_distance(self.creator_embedding, face.normed_embedding)

                        # Convert distance to confidence score (0-1, higher is better match)
                        # distance=0 -> confidence=1.0, distance=threshold -> confidence=0.5, distance>threshold -> confidence<0.5
                        confidence = max(0.0, 1.0 - (distance / (self.threshold * 2.0)))

                        # Track max confidence across all faces in this frame
                        max_confidence_this_frame = max(max_confidence_this_frame, confidence)

                        # Binary match for current frame
                        match = distance <= self.threshold

                        if match:
                            creator_matches.add(i)
                            print(f"[FaceDetector] Face {i} MATCHES creator (distance: {distance:.3f}, confidence: {confidence:.3f}, area: {area:.0f})")
                        else:
                            print(f"[FaceDetector] Face {i} does NOT match (distance: {distance:.3f}, confidence: {confidence:.3f}, area: {area:.0f})")
                ...
                if not hasattr(self, 'face_confidence_history'):
                    self.face_confidence_history = {}

                # Get room-specific confidence history
                if room_id not in self.face_confidence_history:
                    self.face_confidence_history[room_id] = deque(maxlen=5)

                confidence_hist = self.face_confidence_history[room_id]

                # Store max confidence from this frame (highest match among all faces)
                confidence_hist.append(max_confidence_this_frame)

                # Confidence-weighted temporal voting
                # High confidence matches count more toward allowing creator
                # confidence > 0.8 = 2 votes, 0.6-0.8 = 1 vote, < 0.6 = 0.5 vote
                weighted_vote_sum = 0.0
                for conf in confidence_hist:
                    if conf >= 0.8:
                        weighted_vote_sum += 2.0  # Strong match
                    elif conf >= 0.6:
                        weighted_vote_sum += 1.0  # Medium match
                    elif conf >= 0.4:
                        weighted_vote_sum += 0.5  # Weak match
                    # Below 0.4 = 0 votes (no contribution)

                buffer_size = len(confidence_hist)

                # Require weighted vote sum >= 2.0 to allow creator
                # This means: 1 strong match, OR 2 medium matches, OR 4 weak matches
                if buffer_size < 3:
                    # Be more lenient for initial frames
                    creator_allowed = weighted_vote_sum >= 1.0
                    threshold_desc = "lenient (1.0)"
                else:
                    # Standard voting: require weighted sum >= 2.0
                    creator_allowed = weighted_vote_sum >= 2.0
                    threshold_desc = "standard (2.0)"

                print(f"[FaceDetector] Confidence-weighted voting: sum={weighted_vote_sum:.1f}/{buffer_size} frames, threshold: {threshold_desc}, allowed: {creator_allowed}")
        ...
        # Get room-specific temporal data and motion tracker
        if room_id:
            room_masks, room_vote_buf = self._get_room_masks(room_id)
            room_tracker = self._get_room_tracker(room_id) if self.enable_motion_tracking else None
        else:
            # Fallback for legacy calls
            room_masks, room_vote_buf = [], deque(maxlen=5)
            room_tracker = None

        # Motion tracking for fast-moving objects
        if room_tracker is not None:
            # Update tracker with current frame and new detections
            tracked_results = room_tracker.update(frame, new_boxes)

            # Use velocity-expanded boxes for better coverage of fast motion
            rectangles = []
            velocities = []
            for result in tracked_results:
                # Use expanded box for tracking (covers motion path)
                rectangles.append(result['expanded_box'])
                velocities.append(result['velocity'])

            # Convert rectangles to ellipses
            ellipses = []
            for box, velocity in zip(rectangles, velocities):
                ellipse = self.box_to_ellipse(box, W, H)
                # Store velocity info for potential future use
                ellipse['velocity'] = velocity
                ellipses.append(ellipse)

            # Debug logging with motion info
            avg_speed = np.mean([np.sqrt(v[0]**2 + v[1]**2) for v in velocities]) if velocities else 0
            print(f"[FaceDetector] Frame {frame_id}: new_detections={len(new_boxes)}, "
                  f"tracked={len(rectangles)}, ellipses={len(ellipses)}, avg_speed={avg_speed:.1f}px/frame")

        else:
            # Fallback to temporal smoothing (legacy mode without motion tracking)
