...
let GenericForm = class GenericForm extends TsxComponent {
    ...
    render() {
        return (React.createElement(ValidatedForm, { ref: "form", onBlur: (event) => this.onBlurHandler(event) }, this.props.value.map((parameter, inputIndex) => {
            const Component = propertyComponentForType(parameter.type);
            return (React.createElement("div", { key: parameter.name }, parameter.visible && Component && (React.createElement(Component, { value: this.props.value[inputIndex], onInput: (value) => this.onInputHandler(value, inputIndex) }))));
        })));
    }
};
