...
let StreamlabelProperties = class StreamlabelProperties extends Vue {
    ...
    render() {
        return (this.labelSettings && (React.createElement("div", null,
            React.createElement(HFormGroup, { value: this.currentlySelected.name, onInput: (val) => this.handleInput(val), metadata: this.metadata.labelType }),
            Object.keys(this.labelSettings).map((key) => (React.createElement(HFormGroup, { key: key, vModel: this.labelSettings[key], onInput: () => this.debouncedSetSettings(), metadata: this.metadata[key] }))),
            this.labelSettings.format != null && (React.createElement(HFormGroup, { title: $t('Preview') },
                React.createElement("div", { style: "color: var(--title);" }, this.splitPreview.map(line => (React.createElement("div", { key: line }, line)))),
                React.createElement("div", { style: "font-style: italic; opacity: 0.7;" }, $t('Note: Actual label text may take up to 60 seconds to update')))))));
    }
};
__decorate([
    Prop()
...
//# sourceMappingURL=StreamlabelProperties.js.map