...
import { API_NAME_MAP, conditions, newVariation, REGEX_TESTERS, conditionData, } from './alert-box-data';
import { $t } from 'services/i18n';
import { metadata } from 'components/widgets/inputs';
let AlertBoxService = class AlertBoxService extends WidgetSettingsService {
    apiNames() {
    ...
    toggleCustomCode(enabled, data, variation) {
        ...
        Object.keys(newSettings).forEach(type => {
            ...
            const found = variations && variations.find((vari) => variation.id === vari.id);
            if (found) {
    ...
    getMetadata(type, languages, condition) {
        return {
            ...
            ttsSecurity: metadata.spamSecurity({
                ...
                tooltip: $t('This setting helps control text to speech spam, such as "wwwwwwwwwww." ' +
    ...
    patchAfterFetch(data) {
        ...
        return Object.assign(Object.assign({}, rest), { settings: newSettings, custom_defaults: rest.custom });
    }
    ...
    transformSettings(settings) {
        ...
        Object.keys(triagedSettings).forEach(key => {
            if (key === 'subs') {
                triagedSettings['subs'] = this.varifySetting(Object.assign(Object.assign({ showResubMessage: triagedSettings['resubs'].show_message }, triagedSettings['subs']), triagedSettings['resubs']), key);
            }
    ...
    triageSettings(settings) {
        ...
        Object.keys(settings).forEach(key => {
            ...
            REGEX_TESTERS.forEach(test => {
                const newKey = /show/.test(key) && !/show_animation/.test(key)
            ...
            if (['alert_delay', 'moderation_delay'].includes(key)) {
                ...
                const constrainedInterruptDelay = settings.interrupt_mode_delay / 1000 <= 20 ? settings.interrupt_mode_delay / 1000 : 20;
    ...
    varifySetting(setting, type) {
        const { show_message, enabled, showResubMessage } = setting, rest = __rest(setting, ["show_message", "enabled", "showResubMessage"]);
    ...
    unshapeVariation(variation, prefix) {
    ...
    multiplyVariationDuration(variations) {
        variations.forEach(variation => {
            variation.settings.duration = variation.settings.duration * 1000;
        });
        return variations;
    ...
    flattenSettings(settings) {
        ...
        Object.keys(settings).forEach(setting => {
            ...
            if (prefix && prefix !== 'resub') {
                ...
                settingsObj[`${prefix}_variations`] = this.multiplyVariationDuration(settings[setting].variations);
...
AlertBoxService.initialState = WIDGET_INITIAL_STATE;
AlertBoxService = __decorate([
    InheritMutations()
], AlertBoxService);
export { AlertBoxService };
