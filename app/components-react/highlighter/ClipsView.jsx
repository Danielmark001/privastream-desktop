...
export default function ClipsView({ props, emitSetView, }) {
    ...
    function getClipsView(streamId, sortedList, sortedFilteredList) {
        var _a;
        return (<div ref={containerRef} className={styles.clipsViewRoot} onDrop={event => onDrop(event, streamId)}>
        <div className={styles.container}>
          <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
            <header className={styles.header}>
              <button className={styles.backButton} onClick={() => emitSetView(streamId
                ? { view: EHighlighterView.STREAM }
                : { view: EHighlighterView.SETTINGS })}>
                <i className="icon-back"/>
              </button>
              <h1 className={styles.title} onClick={() => emitSetView(streamId
                ? { view: EHighlighterView.STREAM }
                : { view: EHighlighterView.SETTINGS })}>
                {(_a = props.streamTitle) !== null && _a !== void 0 ? _a : $t('All highlight clips')}
              </h1>
            </header>
            <div style={{ padding: '20px', display: 'flex', gap: '8px' }}>
              <Button type="text" icon={<i className="icon-community" style={{ marginRight: 8 }}/>} onClick={shareFeedback}>
                {$t('Share feedback')}
              </Button>
              <PreviewExportButton streamId={streamId} setModal={setModal}/>
            </div>
          </div>
          {sortedList.length === 0 ? (<div style={{ padding: '20px' }}>
              {$t('No clips found')}
              <br />
              <div>
                <AddClip streamId={props.id} addedClips={() => {
                    setClips(sortAndFilterClips(getClips(), props.id, activeFilter));
                }}/>
              </div>
            </div>) : (<>
              {clipsLoaded ? (<>
                  <div className={styles.clipsControls}>
                    ...
                    {streamId &&
                        ...
                        HighlighterService.getClips(HighlighterService.views.clips, props.id)
                            ...
                            .every(clip => { var _a; return (_a = clip.aiInfo.metadata) === null || _a === void 0 ? void 0 : _a.round; }) && (<HighlightGenerator emitSetFilter={filterOptions => {
                  ...
                  <Scrollable className={styles.clipsContainer}>
                    <ReactSortable list={sortedFilteredList} setList={clips => setClipOrder(clips, props.id)} animation={200} filter=".sortable-ignore" onMove={e => {
                      ...
                      {sortedFilteredList.map(({ id }) => {
                        ...
                        return (<div key={clip.path} data-clip-id={id} className={styles.clipItem}>
                            <ClipPreview clipId={id} emitShowTrim={() => {
        ...
        <EditingControls emitSetShowModal={(modal) => {
                setModal({ modal });
            }}/>
        <ClipsViewModal streamId={props.id} modal={modal} onClose={() => setModal(null)} deleteClip={(clipIds, streamId) => setClips(sortAndFilterClips(HighlighterService.getClips(HighlighterService.views.clips, props.id).filter(clip => !clipIds.includes(clip.path)), streamId, 'all'))}/>
      </div>);
    }
...
function AddClip({ streamId, addedClips, }) {
    ...
    return (<Button size="middle" onClick={() => openClips()} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
...
function ClipsLoadingView({ streamId }) {
    const { HighlighterService } = Services;
    const clips = useVuex(() => HighlighterService.getClips(HighlighterService.views.clips, streamId));
    return (<div className={styles.clipLoadingIndicator}>
      <h2>{$t('Loading')}</h2>
      <p>
...
function PreviewExportButton({ streamId, setModal, }) {
    ...
    return (<>
      {hasHighlights && (<Tooltip title={$t('Export detectected timecodes as markers for editing software')} placement="bottom">
          <Button disabled={!hasHighlights} onClick={() => setModal({ modal: 'exportMarkers' })}>
            {$t('Export Markers')}
          </Button>
        </Tooltip>)}
      <Tooltip title={!hasClipsToExport ? $t('Select at least one clip to preview your video') : null} placement="bottom">
        <Button disabled={!hasClipsToExport} onClick={() => setModal({ modal: 'preview' })}>
          {$t('Preview')}
        </Button>
      </Tooltip>
      <Tooltip title={!hasClipsToExport ? $t('Select at least one clip to export your video') : null} placement="bottom">
        <Button disabled={!hasClipsToExport} type="primary" onClick={() => setModal({ modal: 'export' })}>
          {$t('Export')}
        </Button>
      </Tooltip>
    </>);
}
//# sourceMappingURL=ClipsView.jsx.map