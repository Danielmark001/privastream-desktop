...
export default function PreviewModal({ close, streamId, emitSetShowModal, }) {
    ...
    function playPauseButton() {
        if (isPlaying) {
            return React.createElement(PauseButton, null);
        }
        else {
    ...
    if (playlist.length === 0) {
        return (React.createElement("div", null,
            React.createElement("h2", null, $t('Preview')),
            React.createElement("p", null, $t('Select at least one clip to preview your video'))));
    }
    return (React.createElement("div", null,
        React.createElement("h2", null, $t('Preview')),
        React.createElement("p", null, "This is just a preview of your highlight reel. Loading times between clips are possible."),
        React.createElement("div", { style: { position: 'relative', width: '100%', aspectRatio: '16 / 9' } },
            React.createElement("video", { onClick: togglePlay, ref: videoPlayer, className: styles.videoPlayer })),
        React.createElement("div", { style: { display: 'flex', marginTop: '16px', minHeight: '50px' } },
            React.createElement("div", { style: { cursor: 'pointer' }, onClick: () => togglePlay() }, playPauseButton()),
            React.createElement("div", { ref: containerRef, onWheel: handleScroll, className: styles.timeline },
                React.createElement("div", { className: styles.timelineItemWrapper }, playlist.map(({ path }, index) => {
                    let content;
                    if (path === intro.path || path === outro.path) {
                        content = (React.createElement("div", { style: { height: '34px', borderRadius: '6px', overflow: 'hidden' }, onClick: () => {
                                jumpToClip(index);
                            } },
                            React.createElement("video", { id: 'preview-' + index, style: { height: '100%' }, src: path, controls: false, autoPlay: false, muted: true, playsInline: true })));
                    }
                    else {
                        content = (React.createElement(MiniClipPreview, { clipId: path, streamId: streamId, showDisabled: showDisabled, clipStateChanged: (clipId, newState) => {
                    ...
                    return (React.createElement("div", { key: 'preview-mini' + index, id: 'preview-' + index, className: styles.timelineItem, style: {
                            outline: index === currentClipIndex ? '1px solid var(--teal-hover)' : 'unset',
                            outlineOffset: '-2px',
                        } }, content));
                })))),
        React.createElement("div", { className: styles.actionWrapper },
            React.createElement("div", { className: styles.videoDurationWrapper },
                React.createElement("span", null, "0m 0s"),
                React.createElement("span", null, formatSecondsToHMS(getCombinedClipsDuration(playlist.map(clip => {
                    return {
            ...
            React.createElement("div", { style: { display: 'flex', justifyContent: 'space-between', width: '100%' } },
                React.createElement(CheckboxInput, { label: 'Show disabled clips', value: showDisabled, onChange: () => {
                        setShowDisabled(!showDisabled);
                    } }),
                React.createElement(Button, { type: "primary", disabled: playlist.filter(clip => clip.enabled).length === 0, onClick: () => {
