...
export default function StreamCardModal({ streamId, modal, game, onClose, }) {
    ...
    return (<Modal getContainer={`.${styles.streamCardModalRoot}`} onCancel={closeModal} footer={null} width={modalWidth} closable={false} visible={!!showModal || !!v.error} destroyOnClose={true} keyboard={false}>
...
function Feedback(p) {
    ...
    return (<div>
      <TextArea rows={4} maxLength={140} showCount placeholder={$t('Highlights not working? Let us know how we can improve.')} onChange={e => setFeedback(e.target.value)}/>
      <div style={{ textAlign: 'right', marginTop: '24px' }}>
        <Button size="large" type="primary" style={{ marginTop: '14px' }} disabled={!feedback} onClick={leaveFeedback}>
          {$t('Submit')}
        </Button>
      </div>
    </div>);
}
//# sourceMappingURL=StreamCardModal.jsx.map