...
export default function StreamView({ emitSetView }) {
    ...
    return (<div className={cx(styles.streamViewWrapper, showModal && styles.importModalRoot, styles.streamCardModalRoot)} onDrop={event => onDrop(event)}>
      <div style={{ display: 'flex', padding: 20 }}>
        <div style={{ flexGrow: 1 }}>
          <h1 style={{ margin: 0 }}>{$t('My Stream Highlights')}</h1>
        </div>
        <div style={{ display: 'flex', gap: '16px' }}>
          {v.highlighterVersion !== '' && (<div className={styles.uploadWrapper} style={{
      ...
      <Scrollable style={{ flexGrow: 1, padding: '20px 0 20px 20px' }}>
        {highlightedStreams.length === 0 ? (<>No highlight clips created from streams</>) : (Object.entries(groupStreamsByTimePeriod(highlightedStreams)).map(([period, streams]) => streams.length > 0 && (<React.Fragment key={period}>
                  <div className={styles.periodDivider}>{period}</div>
                  <div className={styles.streamcardsWrapper}>
                    {streams.map(stream => (<StreamCard key={stream.id} streamId={stream.id} emitSetView={data => emitSetView(data)}/>))}
                  </div>
                </React.Fragment>)))}
      </Scrollable>

      <Modal getContainer={`.${styles.importModalRoot}`} onCancel={() => {
            var _a;
        ...
        {(showModal === null || showModal === void 0 ? void 0 : showModal.type) === 'upload' && v.highlighterVersion !== '' && (<ImportStreamModal close={closeModal} videoPath={showModal.path} selectedGame={showModal.game} streamInfo={showModal.streamInfo} openedFrom={showModal.openedFrom}/>)}
      </Modal>
    </div>);
}
...
//# sourceMappingURL=StreamView.jsx.map