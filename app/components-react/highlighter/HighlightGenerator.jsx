...
export default function HighlightGenerator({ combinedClipsDuration, game, roundDetails, emitSetFilter, }) {
    ...
    function roundDropdownDetails(roundDetails, game) {
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
          {Object.entries(eventTypeCounts).map(([type, count]) => (<div className={styles.infoTag}>
              {count}
    ...
    return (<h3 className={styles.wrapper} style={{
            color: '#FFFFFF',
            margin: 0,
            fontWeight: 400,
        }}>
      🤖{' '}
      <Translate message={$t('Create highlight video of <roundSelect></roundSelect> with a duration of <minutesSelect></minutesSelect>')} renderSlots={{
            roundSelect: () => (<Select style={selectStyles} mode="multiple" value={selectedRounds} maxTagCount={2} suffixIcon={<DownOutlined style={{ color: '#FFFFFF', fontSize: '12px' }}/>} tagRender={({ value }) => (<span className={styles.tag}>
                  {value === 0
                        ? $t('All Rounds')
                        : $t('Round %{roundNumber}', { roundNumber: value })}
                </span>)} dropdownStyle={dropdownStyles}>
              <div key="all-rounds" className={styles.option}>
                <Checkbox style={checkboxStyles} checked={selectedRounds.includes(0)} onChange={e => {
                    setSelectedRounds(e.target.checked ? [0] : []);
                }}>
                  {$t('All Rounds')}
                </Checkbox>
              </div>
              {roundDetails.map(roundDetails => (<div key={'in-wrapper-round' + roundDetails.round} className={styles.option}>
                  <Checkbox style={checkboxStyles} checked={selectedRounds.includes(roundDetails.round)} onChange={e => {
                        if (e.target.checked) {
                            const newSelection = [
            ...
            minutesSelect: () => (<Select style={{ width: '116px' }} value={targetDuration} onChange={value => setTargetDuration(value)} dropdownStyle={dropdownStyles}>
              {filteredOptions.map(option => (<Option key={option.value} value={option.value} className={styles.option}>
                  {option.label}
                </Option>))}
      ...
      <Button type="text" onClick={() => {
        ...
        }} icon={<span style={{ color: '#666666', fontSize: '20px' }}>&times;</span>} className={styles.resetButton}/>
    </h3>);
}
//# sourceMappingURL=HighlightGenerator.jsx.map