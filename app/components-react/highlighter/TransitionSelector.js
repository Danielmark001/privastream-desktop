...
export default function TransitionSelector() {
    ...
    function getTransitionBlock(transition) {
        const isActive = v.transitionType === transition.type;
        return (React.createElement("div", { style: { textAlign: 'center', fontSize: 11, cursor: 'pointer' }, onClick: () => HighlighterService.actions.setTransition({ type: transition.type }), key: transition.type },
            React.createElement("div", { style: {
                ...
                React.createElement(TransitionIcon, { type: transition.type })),
            React.createElement("div", { style: {
                    margin: '4px 0 8px',
    ...
    return (React.createElement(InputWrapper, { label: "Transition Type" },
        React.createElement("div", { style: { display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' } }, HighlighterService.views.availableTransitions.map(t => {
            return getTransitionBlock(t);
        }))));
}
const TransitionIcon = (p) => {
    if (p.type === 'None')
    ...
    return React.createElement(SvgContainer, { src: iconSrc, style: { width: 36 } });
};
const randomIcon = `
...
//# sourceMappingURL=TransitionSelector.js.map