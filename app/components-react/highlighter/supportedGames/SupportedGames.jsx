...
export default function SupportedGames({ gamesVisible, emitClick, }) {
    ...
    return (<div style={{ display: 'flex' }}>
      {games.slice(0, gamesVisible).map((game, index) => (<div key={game.value + index} onClick={e => emitClick && emitClick(game.value)} className={styles.thumbnail} style={{
                '--rotation': rotation[index],
            }}>
          <img src={game.image} alt={game.label}/>
        </div>))}
      {games.length > gamesVisible && (<Tooltip overlay={<div style={{
                    display: 'flex',
              ...
              {gamesSortedAlphabetical.map((game, index) => (<div onClick={e => emitClick && emitClick(game.value)} key={game.value + index} style={{
                        cursor: 'pointer',
                        display: 'flex',
                        marginBottom: 8,
                        width: '100%',
                  ...
                  <img width={'40px'} height={'40px'} className={styles.thumbnail} style={{ marginRight: 0 }} src={game.image} alt={game.label}/>
...
//# sourceMappingURL=SupportedGames.jsx.map