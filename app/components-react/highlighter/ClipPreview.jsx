...
export default function ClipPreview(props) {
    ...
    return (<div className={styles.previewClip} style={{ opacity: v.clip.enabled ? 1.0 : 0.3 }}>
      <div style={{ height: `${SCRUB_HEIGHT}px`, position: 'relative' }}>
        {!v.clip.deleted && (<img src={clipThumbnail} className={styles.previewImage} style={{
        ...
        <span className={styles.enableButton}>
          <BoolButtonInput tooltip={enabled ? $t('Disable clip') : $t('Enable clip')} tooltipPlacement="top" value={enabled} onChange={setEnabled} checkboxStyles={{
            width: '24px',
            height: '24px',
            fontSize: '14px',
            background: 'white',
        ...
        <div className={styles.previewClipMoving}>
          <div className={styles.controlsContainer}>
            ...
            fontSize: '19px',
                ...
                {isAiClip(v.clip) ? (<ClipPreviewInfo clip={v.clip} game={props.game}/>) : (<div className={styles.highlighterIcon}>
          ...
          <div className={styles.previewClipBottomBar}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              ...
              <Tooltip title={$t('Open file location')} placement="top">
                <Button size="large" className={styles.actionButton} onClick={props.emitOpenFileInLocation}>
...
function FlameHypeScore({ score }) {
    ...
    return (<div className="flex items-center gap-1" style={{ fontSize: '19px' }}>
      {Array.from({ length: fullFlames }).map((_, index) => (<React.Fragment key={'on' + index}>🔥</React.Fragment>))}

      {Array.from({ length: 5 - fullFlames }).map((_, index) => (<span key={'off' + index} style={{ opacity: '0.3' }}>
          🔥
        </span>))}
    </div>);
}
//# sourceMappingURL=ClipPreview.jsx.map