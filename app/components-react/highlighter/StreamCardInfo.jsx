...
export default function StreamCardInfo({ clips, game, }) {
    ...
    return (<Tooltip title={stringsToShow
            .map(item => `${item.emoji} ${item.count} ${item.description}`)
            .join(' | ')}>
      <div className="stream-card-info" style={{
            display: 'block',
            overflow: 'hidden',
            whiteSpace: 'nowrap',
            textOverflow: 'ellipsis',
        }}>
        {stringsToShow.length > 0 ? (stringsToShow.map((item, index) => (<React.Fragment key={index}>
              <span style={{ marginRight: '10px' }}>
                {item.emoji} {item.count} {item.description}
              </span>
            </React.Fragment>))) : (<span>{$t('No events found')}</span>)}
      </div>
    </Tooltip>);
}
//# sourceMappingURL=StreamCardInfo.jsx.map