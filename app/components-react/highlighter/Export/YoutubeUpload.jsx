...
export default function YoutubeUpload(props) {
    ...
    function getYoutubeForm() {
        ...
        <div style={{
          ...
          {v.youtubeLinked && (<div style={{ flexGrow: 1 }}>
              <Form layout="vertical">
                <TextInput label={$t('Title')} value={title} onChange={setTitle}/>
                <TextAreaInput label={$t('Description')} value={description} onChange={setDescription}/>

                <div style={{ marginBottom: '8px' }}> {$t('Privacy Status')}</div>
                <Dropdown overlay={<div className={styles.innerItemWrapper}>
                      {options.map(option => {
                        return (<div className={`${styles.innerDropdownItem} ${option.value === privacy ? styles.active : ''}`} onClick={() => {
                  ...
                  <div className={styles.innerDropdownWrapper} onClick={() => setIsOpen(!isOpen)}>
                    <div className={styles.dropdownText}>
                      {(_a = options.find(option => option.value === privacy)) === null || _a === void 0 ? void 0 : _a.label}
                    </div>
          ...
          {!v.youtubeLinked && (<div style={{ flexGrow: 1 }}>
              ...
              <button style={{ marginTop: 8 }} className="button button--youtube" onClick={() => NavigationService.actions.navigate('PlatformMerge', {
                    platform: 'youtube',
                    highlighter: true,
                })}>
                {$t('Connect')}
        ...
        {((_b = v.youTubeUploadInfo) === null || _b === void 0 ? void 0 : _b.error) && (<Alert style={{ marginBottom: 8, marginTop: 8 }} message={$t('An error occurred while uploading to YouTube')} type="error" closable showIcon afterClose={() => HighlighterService.actions.dismissError()}/>)}
        {v.youtubeLinked && (<Button type="primary" size="large" style={{
                    width: '100%',
                    marginTop: '16px',
    ...
    function getUploadDone() {
        ...
        return (<div>
        <p>
          {$t('Your video was successfully uploaded! Click the link below to access your video. Please note that YouTube will take some time to process your video.')}
        </p>
        <br />
        <a onClick={() => remote.shell.openExternal(url)}>{url}</a>
        <Tooltip placement="right" title={urlCopied ? 'Copied!' : 'Copy URL'}>
          <i className="icon-copy link" style={{ marginLeft: 8, display: 'inline', cursor: 'pointer' }} onClick={() => {
                setUrlCopied(true);
    ...
    return (<div>
      ...
      {v.youtubeLinked && ((_c = v.youTubeUploadInfo) === null || _c === void 0 ? void 0 : _c.uploading) && (<UploadProgress platform={EUploadPlatform.YOUTUBE}/>)}
      {v.youtubeLinked && ((_d = v.youTubeUploadInfo) === null || _d === void 0 ? void 0 : _d.videoId) && getUploadDone()}
    </div>);
}
//# sourceMappingURL=YoutubeUpload.jsx.map