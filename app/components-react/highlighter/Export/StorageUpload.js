...
export default function StorageUpload(p) {
    ...
    if ((uploadInfo === null || uploadInfo === void 0 ? void 0 : uploadInfo.uploading) && uploadInfo.platform === p.platform) {
        return React.createElement(UploadProgress, { platform: p.platform, dense: true });
    }
    return (React.createElement(React.Fragment, null,
        React.createElement(Modal, { visible: showSlidLoginModal, onCancel: () => setShowSlidLoginModal(false), closable: true, destroyOnClose: true, footer: null, title: $t('Sign in with Streamlabs ID'), width: 400 },
            React.createElement(GetSLID, { onLogin: uploadStorage })),
        (uploadInfo === null || uploadInfo === void 0 ? void 0 : uploadInfo.videoId) ? (React.createElement("button", { className: styles.uploadButton, onClick: () => {
                if (!uploadInfo.videoId) {
                    return;
                }
                remote.shell.openExternal(SharedStorageService.views.getPlatformLink(p.platform, uploadInfo.videoId));
            } }, $t('Open'))) : (React.createElement("button", { disabled: uploadInfo === null || uploadInfo === void 0 ? void 0 : uploadInfo.uploading, className: styles.uploadButton, onClick: () => {
                if (!hasSLID) {
...
export function GetSLID(p) {
    ...
    return (React.createElement("div", { className: styles.crossclipContainer },
        React.createElement("h2", { className: styles.signUpTitle }, $t('This feature requires a Streamlabs ID')),
        React.createElement("button", { className: "button button--action", style: { width: '300px', margin: '32px' }, onClick: () => clickLink(true) }, $t('Sign up for Streamlabs ID')),
        React.createElement("span", { className: styles.login },
            React.createElement(Translate, { message: "Already have a Streamlabs ID? <link>Login</link>" },
                React.createElement("a", { slot: "link", onClick: () => clickLink() })))));
}
//# sourceMappingURL=StorageUpload.js.map