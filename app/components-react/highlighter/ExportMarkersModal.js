...
export default function ExportMarkersModal({ close, streamId, }) {
    ...
    return (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' } },
        React.createElement("h2", null, $t('Export Markers')),
        React.createElement("p", null, "Select the video editing software you want to export markers for."),
        React.createElement(Select, { style: { width: '100%' }, value: markersFormat, onChange: value => setMarkersFormat(value) }, availableMarkersFormats.map(option => (React.createElement(Option, { key: option.value, value: option.value }, option.label)))),
        markersFormat !== 'youtube' && (React.createElement(Checkbox, { checked: startFromHour, onChange: e => setStartFromHour(e.target.checked), style: { marginTop: '10px' } }, $t('Timeline starts from 01:00:00 (default)'))),
        markersFormat !== 'youtube' && (React.createElement(Checkbox, { checked: exportRanges, onChange: e => setExportRanges(e.target.checked), style: { marginTop: '6px', marginLeft: '0px' } }, $t('Export full highlight duration as marker range'))),
        React.createElement(Button, { type: "primary", style: { marginTop: '20px', width: '100%' }, loading: exporting, onClick: exportMarkers }, $t('Export'))));
}
//# sourceMappingURL=ExportMarkersModal.js.map