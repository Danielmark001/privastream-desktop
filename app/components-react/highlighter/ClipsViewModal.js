...
export default function ClipsViewModal({ streamId, modal, onClose, deleteClip, }) {
    ...
    return (React.createElement(Modal, { getContainer: `.${styles.clipsViewRoot}`, onCancel: closeModal, footer: null, width: modalWidth, closable: false, visible: !!showModal || !!v.error, destroyOnClose: true, keyboard: false },
        !!v.error && React.createElement(Alert, { message: v.error, type: "error", showIcon: true }),
        inspectedClip && showModal === 'trim' && (React.createElement(ClipTrimmer, { clip: inspectedClip, streamId: streamId })),
        ...
        showModal === 'exportMarkers' && streamId && (React.createElement(ExportMarkersModal, { close: closeModal, streamId: streamId })),
        inspectedClip && showModal === 'remove' && (React.createElement(RemoveModal, { key: `remove-${inspectedClip.path}`, close: closeModal, clip: inspectedClip, streamId: streamId, deleteClip: deleteClip, removeType: 'clip' }))));
}
//# sourceMappingURL=ClipsViewModal.js.map