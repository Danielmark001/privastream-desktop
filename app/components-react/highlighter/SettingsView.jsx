...
export default function SettingsView({ emitSetView, close, }) {
    ...
    return (<div className={styles.settingsViewRoot}>
      <div style={{ display: 'flex', padding: 20 }}>
        ...
        <div style={{ display: 'flex', gap: '16px' }}>
          {aiHighlighterFeatureEnabled && (<Button type="primary" onClick={() => emitSetView({ view: EHighlighterView.STREAM })}>
              {$t('Stream Highlights')}
            </Button>)}
      ...
      <Scrollable style={{ flexGrow: 1, padding: '20px 20px 20px 20px', width: '100%' }}>
        <div className={styles.innerScrollWrapper}>
          <div className={styles.cardWrapper}>
            {aiHighlighterFeatureEnabled && (<div className={styles.highlighterCard}>
                <div className={styles.cardHeaderbarWrapper}>
                  <div className={styles.cardHeaderbar}>
                    ...
                    <SupportedGames gamesVisible={6} emitClick={() => {
                ...
                <p style={{ margin: 0 }}>
                  ...
                  {v.highlighterVersion !== '' && (<span>
                      {$t('The AI Highlighter App can be managed in the Apps Manager tab or in Settings > Installed apps.')}
                    </span>)}
                </p>

                {v.highlighterVersion !== '' ? (<SwitchInput style={{ margin: 0, marginLeft: '-10px' }} size="default" value={v.useAiHighlighter} onChange={toggleUseAiHighlighter}/>) : (<Button style={{ width: 'fit-content' }} type="primary" onClick={() => {
            ...
            : $t('Built-in manual highlighter')}
              ...
              <div style={{ display: 'flex', gap: '16px' }}>
                {!v.isStreaming && !correctlyConfigured && (<div className={styles.settingSection}>
                    {correctlyConfigured
                ? completedStepHeading($t('Configure the replay buffer'))
                : incompleteStepHeading($t('Configure the replay buffer'))}
                    {correctlyConfigured ? (<div>{$t('The replay buffer is correctly configured')}</div>) : (<Button onClick={configure}>{$t('Configure')}</Button>)}
                  </div>)}
                {v.isStreaming && (<div className={styles.settingSection} style={{ width: '100%' }}>
                    <p>{$t('End your stream to change the Hotkey or the replay duration.')}</p>
                  </div>)}

                {!v.isStreaming && (<div className={styles.settingSection}>
                ...
                : incompleteStepHeading($t('Set a hotkey to capture replays'))}
                    {hotkey && (<HotkeyBinding style={{ width: 'calc(100% + 10px)', marginLeft: '-10px' }} showLabel={false} hotkey={hotkey} binding={(_a = hotkey.bindings[0]) !== null && _a !== void 0 ? _a : null} onBind={binding => {
                ...
                {!v.isStreaming && (<div className={styles.settingSection} style={{ width: '100%' }}>
                    ...
                    <Form layout="inline">
                      <SliderInput style={{ width: 'calc(100% + 14px)', marginLeft: '-10px' }} label={null} value={v.settingsValues.Output.RecRBTime} onChange={setReplayTime} min={1} max={120} step={1} debounce={200} hasNumberInput={false} tooltipPlacement="top" tipFormatter={v => `${v}s`}/>
                    </Form>
                  </div>)}
              </div>
            </div>
