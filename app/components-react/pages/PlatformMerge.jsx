...
export default function PlatformMerge(p) {
    ...
    function LoginStep() {
        ...
        <button style={{ marginTop: '24px' }} className={`button button--${platform}`} disabled={loading} onClick={mergePlatform}>
    ...
    function OverlayStep() {
        ...
        <br />
        <button style={{ marginTop: '24px' }} className="button button--action" disabled={loading || authInProgress} onClick={installOverlay}>
          <i className={loading ? 'fas fa-spinner fa-spin' : 'icon-themes'}/>
          {$t('Install %{overlayName}', { overlayName: p.params.overlayName })}
        </button>
      </>);
    }
    return (<div className={p.className} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
...
//# sourceMappingURL=PlatformMerge.jsx.map