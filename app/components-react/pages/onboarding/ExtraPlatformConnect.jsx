    ...
    return (<div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className={styles.container} style={{ height: '50%' }}>
        ...
        <div className="section">
          <Form>
            <TextInput label={$t('Stream Key')} value={key} onChange={setKey} isPassword={true} uncontrolled={false}/>
          </Form>
        </div>

        <p>
          <button className="button button--action" onClick={onFinish} disabled={!key.trim().length}>
            {$t('Finish')}
          </button>
        </p>
...
//# sourceMappingURL=ExtraPlatformConnect.jsx.map