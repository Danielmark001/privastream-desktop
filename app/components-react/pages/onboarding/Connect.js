...
export function Connect() {
    ...
    if (selectedExtraPlatform) {
        return (React.createElement(ExtraPlatformConnect, { selectedExtraPlatform: selectedExtraPlatform, setExtraPlatform: setExtraPlatform }));
    }
    function onSkip() {
    ...
    return (React.createElement("div", { className: styles.pageContainer },
        React.createElement("div", { className: styles.container },
            React.createElement("h1", { className: commonStyles.titleContainer, style: { marginTop: 0 } }, title),
            isSignup ? (React.createElement(Signup, { onSignupLinkClick: () => setIsSignup(false), onSuccess: afterLogin })) : (React.createElement(React.Fragment, null,
                !isRelog && React.createElement("p", { style: { marginBottom: 20 } }, $t('Log in with email/password')),
                isRelog && (React.createElement("h3", { style: { marginBottom: '16px' } }, $t('Your login has expired. Please reauthenticate to continue using Streamlabs Desktop.'))),
                React.createElement("div", { className: styles.streamlabsPlatformContainer },
                    React.createElement(PlatformButton, { platform: "streamlabs", loading: loading, disabled: loading || authInProgress, onClick: () => authPlatform('streamlabs', afterLogin) },
                        React.createElement(Translate, { message: $t('Log in with <span>%{platform}</span>', {
                                platform: 'Streamlabs ID',
                            }) },
                            React.createElement("span", { slot: "span", style: { fontWeight: 'bold' } })))),
                React.createElement("div", { className: styles.thirdPartyPlatforms },
                    React.createElement("p", null, $t('Log in with a platform')),
                    React.createElement("div", { className: styles.thirdPartyPlatformsContainer }, platforms.map((platform) => (React.createElement(PlatformButton, { platform: platform, disabled: loading || authInProgress, loading: loading, onClick: () => authPlatform(platform, afterLogin), key: platform, logoSize: ['twitter', 'tiktok', 'youtube', 'kick'].includes(platform) ? 15 : undefined },
                        React.createElement(Translate, { message: $t('Log in with <span>%{platform}</span>', {
                                platform: platformLabels(platform),
                            }) },
                            React.createElement("span", { slot: "span" })))))),
                    React.createElement("div", { className: styles.extraPlatformsContainer },
                        shouldAddTrovo && (React.createElement(PlatformIconButton, { name: EPlatform.Trovo, platform: EPlatform.Trovo, disabled: loading || authInProgress, loading: loading, onClick: () => authPlatform(EPlatform.Trovo, afterLogin), title: $t('Log in with %{platform}', {
                                platform: platformLabels(EPlatform.Trovo),
                            }) })),
                        extraPlatforms.map(platform => (React.createElement(PlatformIconButton, { name: platform.value, key: platform.value, logo: platform.image, disabled: loading || authInProgress, loading: loading, onClick: () => onSelectExtraPlatform(platform.value), title: $t('Log in with %{platform}', {
                                platform: platform.label,
                            }) }))))),
                React.createElement("div", { className: styles.signupLinkContainer },
                    React.createElement(Translate, { message: $t("Don't have an account yet? <span>Sign up</span>") },
                        React.createElement("a", { slot: "span", onClick: () => setIsSignup(true) })))))),
        React.createElement("div", { className: styles.svgBackgrounds },
            React.createElement(SVGOvalLeftBackground, null),
            React.createElement(SVGOvalRightBackground, null))));
}
const SVGOvalRightBackground = () => (React.createElement("svg", { id: "oval-right", width: "692", height: "524", viewBox: "0 0 692 524", fill: "none", xmlns: "http://www.w3.org/2000/svg" },
    React.createElement("path", { d: "M156.333 -77.6233C116.026 -196.226 142.587 -330.779 214.729 -416.705C286.872 -502.631 404.88 -539.961 522.904 -543.795C640.928 -547.629 759.072 -518.127 862.158 -451.659C965.244 -385.191 1053.09 -281.887 1093.81 -163.927C1134.63 -46.1289 1128.41 86.1639 1067.24 175.508C1006.34 264.821 890.687 310.864 772.152 315.502C653.515 320.301 532.101 283.535 417.766 213.68C303.613 143.955 196.641 40.9795 156.333 -77.6233Z", fill: "#09161D" })));
const SVGOvalLeftBackground = () => (React.createElement("svg", { id: "oval-left", width: "669", height: "620", viewBox: "0 0 669 620", fill: "none", xmlns: "http://www.w3.org/2000/svg" },
    React.createElement("path", { d: "M518.105 430.239C581.518 535.15 584.359 669.337 533.428 766.607C482.498 863.876 377.531 924.319 265.52 952.746C153.509 981.173 34.39 977.76 -78.092 935.854C-190.574 893.948 -296.219 813.634 -359.887 709.424C-423.619 605.389 -445.437 477.631 -405.705 379.395C-366.237 281.247 -265.345 212.971 -153.015 183.668C-40.6209 154.19 83.1482 163.859 207.092 206.643C330.837 249.341 454.693 325.327 518.105 430.239Z", fill: "#09161D" })));
