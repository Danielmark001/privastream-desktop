...
export function MacPermissions() {
    ...
    return (<div style={{ width: '650px', margin: '100px auto' }}>
      ...
      <div style={{ fontSize: '16px', marginTop: '16px' }}>
        <div>
          {$t('Microphone')}
          {permissions.micPermission && (<i className="fa fa-check" style={{ marginLeft: '8px', color: '#80F5D2' }}/>)}
        </div>
        <div>
          {$t('Webcam')}
          {permissions.webcamPermission && (<i className="fa fa-check" style={{ marginLeft: '8px', color: '#80F5D2' }}/>)}
        </div>
      </div>
      <button className="button button--action" style={{ float: 'right' }} onClick={() => next()} disabled={!permissions.webcamPermission || !permissions.micPermission}>
        {$t('Continue')}
      </button>
    </div>);
}
//# sourceMappingURL=MacPermissions.jsx.map