...
export default function Onboarding() {
    ...
    return (React.createElement("div", { className: cx(styles.onboardingContainer) },
        React.createElement(TopBar, null),
        React.createElement("div", { className: styles.onboardingContent },
            React.createElement(Scrollable, { style: { height: '100%' } },
                React.createElement(Component, null),
                !currentStep.hideButton && React.createElement(ActionButton, null))),
        shouldShowFooter && (React.createElement(Footer, { onSkip: skip, currentStep: currentStep, currentStepIndex: currentStepIndex, isProcessing: processing, totalSteps: totalSteps }))));
}
function Footer({ currentStep, totalSteps, onSkip, isProcessing, currentStepIndex }) {
    ...
    return (React.createElement("div", { className: cx(styles.footer, { [styles.footerPrime]: isPrimeStep }) },
        React.createElement("div", { className: cx(styles.progress, { [styles.progressWithSkip]: currentStep.isSkippable }) },
            React.createElement(Progress, { showInfo: false, steps: totalSteps, percent: percent })),
        currentStep.isSkippable && (React.createElement("div", { className: styles.skip },
            React.createElement("button", { className: cx('button button--trans', styles.linkButton, commonStyles.onboardingButton), onClick: onSkip, disabled: isProcessing }, $t('Skip'))))));
}
function TopBarLogo({ component }) {
    return React.createElement(StreamlabsDesktopIcon, null);
}
function TopBar() {
    const component = useModule(OnboardingModule).currentStep.component;
    return (React.createElement("div", { className: cx(styles.topBarContainer, {
            [styles.topBarContainerPrime]: component === 'Prime',
        }) },
        React.createElement(TopBarLogo, { component: component })));
}
function ActionButton() {
    ...
    if (currentStep.hideButton)
        return null;
    const isPrimeStep = currentStep.component === 'Prime';
    return (React.createElement("div", { style: { flex: 1, justifyContent: 'center', display: 'flex', alignItems: 'center' } },
        React.createElement(Button, { className: cx(styles.actionButton, { 'button--prime': isPrimeStep }), type: "primary", shape: "round", size: "large", onClick: () => next(), disabled: processing }, isPrimeStep ? $t('Go Ultra') : $t('Continue'))));
}
