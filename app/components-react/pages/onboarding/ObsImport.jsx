...
export function ObsImport() {
    ...
    return (<div style={{ width: '100%' }}>
      ...
      {!importing && <PreImport />}
      {importing && (<div className={styles.progressBar}>
...
function PreImport() {
    ...
    return (<div>
      {profiles.length > 1 && (<div style={{ width: 400, margin: 'auto', textAlign: 'center' }}>
          <span className={styles.profileSelectTitle}>{$t('Select an OBS profile to import')}</span>
          <Form layout="inline">
            <ListInput style={{ width: '100%' }} options={profiles.map(p => ({ label: p, value: p }))} value={selectedProfile} onChange={setSelectedProfile} allowClear={false}/>
          </Form>
        </div>)}
      <button className={commonStyles.optionCard} style={{ margin: 'auto', marginTop: 24 }} onClick={() => __awaiter(this, void 0, void 0, function* () {
            setProcessing(true);
            if (!isObs) {
                yield alertAsync({
                    title: $t('Twitch Studio Import'),
                    content: (<p>
                  {$t('Importing from Twitch Studio is an experimental feature under active development. Some source types are unable to be imported, and not all settings will be carried over.')}
                </p>),
                    okText: $t('Start'),
                    okType: 'primary',
                });
...
function FeatureCards() {
    ...
    return (<div className={styles.container}>
      {Object.keys(featuresMetadata).map(feature => {
            ...
            return (<div className={styles.card} key={feature}>
            <div>
              <h3>{data.title}</h3>
...
//# sourceMappingURL=ObsImport.jsx.map