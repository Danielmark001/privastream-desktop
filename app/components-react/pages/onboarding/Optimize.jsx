...
export function Optimize() {
    ...
    return (<div>
      ...
      {(optimizingState === 'initial' || optimizingState === 'error') && (<div style={{ width: '60%', margin: 'auto', textAlign: 'center' }}>
      ...
      {optimizingState === 'running' ? (<div style={{ margin: 'auto', marginTop: 24, width: '80%' }}>
          <AutoProgressBar percent={percentage * 100} timeTarget={1000 * 60}/>
          <span>{stepInfo && stepInfo.summary}</span>
        </div>) : (<button className={commonStyles.optionCard} onClick={() => optimizingState === 'initial' ? optimize() : closeOnError()} style={{ margin: 'auto', marginTop: 24 }}>
          <h2 style={{ color: 'var(--action-button-text)' }}>{optimizingState === 'initial' ? $t('Start') : $t('Ok')}</h2>
        </button>)}
    </div>);
}
//# sourceMappingURL=Optimize.jsx.map