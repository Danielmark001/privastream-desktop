...
export function ThemeSelector() {
    ...
    return (<div style={{ width: '100%' }}>
      ...
      <div>
        {!installing ? (<div className={styles.container}>
            {bigPreview && detailTheme && (<>
                <div className={styles.themesSidebar}>
                  <h1>{$t('Other Themes')}</h1>

                  {getFilteredMetadata().map(theme => (<div className={styles.cell} onClick={() => focusTheme(theme)} key={theme.data.name}>
                ...
                <div className={cx(styles.detailPanel, styles.right)}>
                  <div className={styles.detailHeader}>
                    {detailTheme.data.designer && (<img className={styles.designerLogo} src={detailTheme.data.designer.avatar}/>)}
                    <div className={styles.themeDetails}>
                      <h1>{detailTheme.data.name}</h1>
                      {detailTheme.data.designer && (<Translate message={$t('by <designer>%{designerName}</designer>', {
                        designerName: detailTheme.data.designer.name,
                    })}>
                          <span slot="designer" className={cx(styles.designerName, {
                    ...
                    <Button shape="round" size="large" type="primary" onClick={e => installTheme(e)} disabled={installing}>
                      {$t('Install')}
                    </Button>
                  </div>
                  <div className={styles.previewGrid}>
                    <ThemePreview src={bigPreview} className={styles.bigPreview} focusPreview={() => { }}/>
                    {previewImages(detailTheme).map(src => {
                    return (<ThemePreview key={src} src={src} className={cx(styles.preview, { [styles.active]: src === bigPreview })} focusPreview={() => setBigPreview(src)}/>);
                })}
                  </div>
                </div>
              </>)}
          </div>) : (<div style={{ margin: 'auto', marginTop: 24, width: '80%' }}>
...
function ThemePreview(p) {
    ...
    return isVideo ? (<video autoPlay muted loop src={p.src} className={p.className} onClick={e => {
            e.stopPropagation();
            p.focusPreview();
        }}/>) : (<img src={p.src} className={p.className} onClick={e => {
            e.stopPropagation();
            p.focusPreview();
        }}/>);
}
