...
export function HardwareSetup() {
    ...
    return (<div style={{
      ...
      <div className={styles.contentContainer}>
        <DisplaySection />
        {!!v.videoDevices.length && (<Form layout="vertical" style={{ width: 300 }}>
            <ListInput label={$t('Webcam')} options={v.videoDevices} value={v.selectedVideoDevice} onChange={setVideoDevice} allowClear={false}/>
            <ListInput label={$t('Visual Preset')} options={SourceFiltersService.views.presetFilterOptionsReact} value={v.presetFilterValue} onChange={setPresetFilter} allowClear={false}/>
            <ListInput label={$t('Microphone')} options={v.audioDevices} value={v.selectedAudioDevice} onChange={val => DefaultHardwareService.actions.setDefault('audio', val)} allowClear={false}/>
          </Form>)}
      </div>
    </div>);
}
function DisplaySection() {
    ...
    if (v.selectedVideoSource && v.videoDevices.length) {
        return (<div className={cx(styles.display, 'section')}>
        <Display sourceId={v.selectedVideoSource.sourceId} renderingMode={0}/>
...
//# sourceMappingURL=HardwareSetup.jsx.map