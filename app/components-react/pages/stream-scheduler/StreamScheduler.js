...
export default function StreamSchedulerPage() {
    ...
    return (React.createElement(StreamSchedulerCtx.Provider, { value: controller },
        React.createElement(StreamScheduler, null)));
}
function StreamScheduler() {
    const { store } = useStreamScheduler();
    const isEventsLoaded = store.useState(s => s.isEventsLoaded);
    return (React.createElement(Scrollable, { className: cx(css.streamSchedulerPage) },
        React.createElement(Spin, { tip: "Loading...", spinning: !isEventsLoaded },
            React.createElement(SchedulerCalendar, null)),
        React.createElement(EventSettingsModal, null)));
}
function SchedulerCalendar() {
    ...
    function dateCellRender(date) {
        ...
        return (React.createElement("div", { "data-role": "day", onClick: () => showNewEventModal(selectedPlatform, date.valueOf()) }, dayEvents.map(renderEvent)));
    }
    function renderEvent(event) {
        const time = moment(event.date).format('hh:mma');
        return (React.createElement("p", { key: event.id, title: event.title, className: cx({
                [css.event]: true,
            ...
            React.createElement("span", { className: css.eventTime }, time),
            " \u00A0",
            React.createElement("span", { className: css.eventTitle }, event.title)));
    }
    ...
    return (React.createElement("div", { onClick: onCalendarClick },
        React.createElement(Calendar, { dateCellRender: dateCellRender, validRange: [minDate, maxDate] })));
}
function EventSettingsModal() {
    ...
    return (React.createElement(Modal, { title: title, visible: isModalVisible, onOk: submit, onCancel: closeModal, afterClose: closeModal, destroyOnClose: true, footer: React.createElement(ModalButtons, null), getContainer: `.${css.streamSchedulerPage}` },
        React.createElement(Form, { form: form },
            React.createElement(Spin, { spinning: isLoading },
                canChangePlatform && (React.createElement(React.Fragment, null,
                    React.createElement(ListInput, { label: $t('Platform'), name: "platform", value: selectedPlatform, options: platforms.map(platform => ({
                            value: platform,
                            label: getPlatformDisplayName(platform),
                        })), onChange: platform => showNewEventModal(platform) }),
                    selectedPlatform === 'facebook' && (React.createElement("span", { className: "whisper" }, $t('Please note that while you can schedule streams to Facebook, some will not appear on this calendar due to API limitations'))))),
                React.createElement(TimeInput, { name: "time", label: $t('Time'), value: time, onChange: setTime }),
                selectedPlatform === 'youtube' && (React.createElement(YoutubeEditStreamInfo, { layoutMode: "singlePlatform", isUpdateMode: isUpdateMode, isScheduleMode: true, value: ytSettings, onChange: newSettings => updatePlatform('youtube', newSettings) })),
                selectedPlatform === 'facebook' && (React.createElement(FacebookEditStreamInfo, { layoutMode: "singlePlatform", isUpdateMode: isUpdateMode, isScheduleMode: true, value: fbSettings, onChange: newSettings => updatePlatform('facebook', newSettings) }))))));
}
function ModalButtons() {
    ...
    return (React.createElement(Row, null,
        React.createElement(Col, { flex: '50%', style: { textAlign: 'left' } }, shouldShowRemove && (React.createElement(Button, { danger: true, onClick: onDeleteClick }, $t('Delete')))),
        React.createElement(Col, { flex: '50%' },
            shouldShowGoLive && (React.createElement(Button, { onClick: goLive, type: "primary" }, $t('Go Live'))),
            shouldShowSave && (React.createElement(Button, { type: "primary", onClick: submit, disabled: isLoading }, $t('Save'))),
            shouldShowSchedule && (React.createElement(Button, { type: "primary", onClick: submit, disabled: isLoading }, $t('Schedule'))))));
}
//# sourceMappingURL=StreamScheduler.js.map