...
export default function ThemeAudit(p) {
    ...
    function renderStat(label, displayValue, type) {
        ...
        return (<span style={{ color, marginRight: 8 }}>
    ...
    return (<div style={{ width: '100%', display: 'flex' }} className={cx(styles.themeAuditRoot, p.className)}>
      <Scrollable style={{ flexGrow: 1, padding: 20 }}>
        <h1>Theme Audit: {(_a = SceneCollectionsService.activeCollection) === null || _a === void 0 ? void 0 : _a.name}</h1>
        ...
        <Tabs>
          <Tabs.TabPane tab="Overview" key="overview">
            <div className="section">
              ...
              <div>
                {renderStat('Webcam', hasWebcam ? 'Present' : 'Missing', hasWebcam ? 'OK' : 'CRITICAL')}
              </div>
              <div>
                {renderStat('Game Capture', hasGameCapture ? 'Present' : 'Missing', hasGameCapture ? 'OK' : 'CRITICAL')}
              </div>
            </div>
          </Tabs.TabPane>
          <Tabs.TabPane tab="Media Sources" key="media">
            {mediaInfo && (<div>
                ...
                var _a, _b, _c;
                return (<div key={scene.name} className="section">
                    <h3>{scene.name}</h3>
                    <div>
                      {renderNumericStat('Media Source Count', ((_a = grouped[scene.name]) !== null && _a !== void 0 ? _a : []).length.toString(), ((_b = grouped[scene.name]) !== null && _b !== void 0 ? _b : []).length, [2, 4])}
                    </div>
                    {((_c = grouped[scene.name]) !== null && _c !== void 0 ? _c : []).map(info => (<div key={info.id} style={{ padding: '5px 0' }}>
        ...
        <Modal visible={!!inspectedSource} destroyOnClose={true} closable={false} onCancel={() => inspect(null)} onOk={() => inspect(null)} width={780}>
          {inspectedSource && (<div>
...
//# sourceMappingURL=ThemeAudit.jsx.map