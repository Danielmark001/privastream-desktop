    ...
    return (<div className={styles.sideBar}>
      <div>
        ...
        <Scrollable className={styles.layouts}>
          {Object.values(ELayout).map(layout => (<img key={layout} className={currentLayout === layout ? styles.active : ''} onClick={() => setCurrentLayout(layout)} src={layoutImage(layout)}/>))}
        </Scrollable>
      </div>
      <ElementList />
    </div>);
...
function ElementList() {
    ...
    return (<div className={styles.elementList}>
      ...
      <Scrollable className={styles.elementContainer}>
        {Object.keys(ELayoutElement).map((element) => (<div draggable key={element} className={styles.elementCell} onDragEnd={(e) => handleElementDrag(e, ELayoutElement[element])}>
