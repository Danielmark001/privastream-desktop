...
export default function LayoutEditor() {
    ...
    return (<div style={{ flexDirection: 'column', width: '100%' }}>
      ...
      <div className={styles.editorContainer}>
        <SideBar />
        <div className={cx(styles.templateContainer, styles[LayoutService.views.className(currentLayout)])}>
          <DisplayedLayout />
        </div>
      </div>
...
function DisplayedLayout() {
    ...
    return (<>
      {['1', '2', '3', '4', '5', '6'].map((slot) => (<div className={classForSlot(slot)} id={slot} key={slot} draggable={elementInSlot(slot) && canDragSlot} onDragEnter={() => setHighlightedSlot(slot)} onDragExit={() => setHighlightedSlot(null)} onDragEnd={(e) => handleElementDrag(e, ELayoutElement[elementInSlot(slot)])}>
          <span>{LayoutService.views.elementTitle(elementInSlot(slot))}</span>
          {elementInSlot(slot) === ELayoutElement.Browser && (<div className={styles.urlInput}>
              <TextInput value={browserUrl} onChange={setBrowserUrl} onFocus={() => setCanDragSlot(false)} onBlur={() => setCanDragSlot(true)} placeholder={$t('Enter Target URL')} nowrap/>
            </div>)}
        </div>))}
    </>);
}
