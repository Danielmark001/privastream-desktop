...
export default function AddTabModal() {
    ...
    function Footer() {
        ...
        <button className="button button--action" onClick={createTab} disabled={!icon || !name} style={{ marginLeft: '8px' }}>
          {$t('Save New Tab')}
        </button>
      </>);
    }
    return (<ModalLayout footer={<Footer />} wrapperStyle={{ width: '390px', height: '270px' }}>
      <Form>
        <CardInput value={icon} onInput={setIcon} options={ICONS} isIcons={true} style={{ borderRadius: 4, margin: 4 }} itemWidth={48} itemHeight={48} nowrap/>
        <TextInput label={$t('Name')} value={name} onChange={setName} style={{ marginTop: '10px', marginLeft: '-80px', marginRight: '3px' }} uncontrolled={false}/>
      </Form>
    </ModalLayout>);
}
//# sourceMappingURL=AddTabModal.jsx.map