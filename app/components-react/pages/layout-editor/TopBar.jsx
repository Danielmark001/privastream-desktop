...
export default function TopBar() {
    ...
    return (<Form className={styles.topBar}>
      <img className={styles.arrow} src={require('../../../../media/images/chalk-arrow.png')}/>
      <button className="button button--action" style={{ margin: '0 16px' }} onClick={() => setShowModal(true)}>
        {$t('Add Tab')}
      </button>
      <ListInput label="" style={{ width: '150px', marginBottom: 0 }} value={currentTab} onChange={setCurrentTab} options={tabOptions} tooltip={{ title: $t('Current Tab'), placement: 'bottom' }}/>
      {currentTab !== 'default' && (<Tooltip title={$t('Delete Current Tab')} placement="bottom">
          <button className={cx('button button--warn', styles.removeButton)} onClick={removeCurrentTab}>
