...
export function GoalCard(p) {
    ...
    return (<div className={styles.card} key={type}>
      <strong>{title}</strong>
      {daysLeft !== Infinity && !goalFinished && (<span className={styles.whisper}>{$t('%{daysLeft} days left', { daysLeft })}</span>)}
      {p.goal.progress != null && (<Progress percent={Math.floor((p.goal.progress / p.goal.total) * 100)} showInfo={false} steps={p.goal.total} strokeWidth={16}/>)}
      {p.goal.progress != null && manuallyProgressedGoal && !goalFinished ? (<Button onClick={incrementCustomGoal} type="primary">
          {$t('Progress')}
        </Button>) : (<img src={image}/>)}
      {goalFinished && <span>{goalFinishedText}</span>}
      {p.goal.progress == null && <Button onClick={addGoal}>{$t('Add Goal')}</Button>}
      {p.goal.progress != null && (<i onClick={removeGoal} className={cx('icon-close', styles.closeIcon)}/>)}
    </div>);
}
export function UniversityCard(p) {
    ...
    return (<div className={styles.card} style={{ minWidth: '580px' }}>
...
function UniversityProgress(p) {
    ...
    return (<div>
      <span>
        ...
        <Progress percent={p.progress.total_progress} showInfo={false} style={{ marginRight: '16px' }}/>
        <h3>{(_a = p.progress.stopped_at) === null || _a === void 0 ? void 0 : _a.title}</h3>
        <span>{(_b = p.progress.stopped_at) === null || _b === void 0 ? void 0 : _b.description}</span>
      </span>
    </div>);
}
export function ContentHubCard() {
    ...
    return (<div className={styles.card} style={{ minWidth: '580px' }}>
      ...
      <img className={styles.cardImage} src="https://slobs-cdn.streamlabs.com/media/grow/content_hub.png"/>
    </div>);
}
export function PlatformCard(p) {
    ...
    return (<div className={styles.card}>
      ...
      {followers != null ? (<span>{$t('%{followers} followers', { followers })}</span>) : (<Button onClick={platformMerge}>{$t('Connect')}</Button>)}
    </div>);
}
export function MultistreamCard() {
    ...
    return (<div className={cx(styles.card, styles.primeCard)} onClick={getPrime}>
...
//# sourceMappingURL=Cards.jsx.map