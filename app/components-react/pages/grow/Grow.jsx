...
export default function Grow() {
    ...
    return (<div className={styles.goalTabContainer}>
...
function MyGoals(p) {
    ...
    return (<div className={styles.myGoals}>
      <h2>{$t('My Goals')}</h2>
      ...
      <div className={styles.goalsContainer}>
        {mappedGoals.map(goal => (<GoalCard goal={goal} key={goal.type}/>))}
        {appendedOptions.map(goal => (<GoalCard goal={goal} key={goal.type} showGoalModal={() => setShowGoalModal(true)}/>))}
      </div>
...
function AddGoalModal(p) {
    ...
    return (<Modal visible={p.visible} getContainer={`.${styles.goalTabContainer}`} onOk={form.submit} onCancel={() => p.setShowGoalModal(false)} title={$t('Add Goal')}>
      <Form form={form} onFinish={addGoal}>
        <ListInput label={$t('Goal Type')} options={goalTypes} value={goalType} defaultValue="custom" onChange={setGoalType} rules={[{ validator: uniqueGoalValidator }]}/>
        {goalType === 'custom' && (<TextInput label={$t('Goal Title')} value={goalTitle} onChange={setGoalTitle} uncontrolled={false} placeholder={'My Goal'} rules={[{ max: 50 }]} required/>)}
        {goalType === 'custom' && (<NumberInput label={$t('Goal Total')} value={goalTotal} onChange={setGoalTotal} defaultValue={10} uncontrolled={false} rules={[{ min: 1, max: 50 }]} required/>)}
      </Form>
    </Modal>);
}
function MyCommunity(p) {
    ...
    const reachableFollowing = p.platforms
        ...
        .reduce((count, current) => count + current.followers, 0);
    return (<div className={styles.myCommunity}>
      <h2>
      ...
      <div className={styles.communityContainer}>
        {p.platforms.map(platform => (<PlatformCard platform={platform} key={platform.icon}/>))}
      </div>
    </div>);
}
const STATS_TO_MAP = () => [
...
function StreamPulse(p) {
    ...
    return (<div className={styles.streamPulse}>
      <h2>{$t('Stream Pulse')}</h2>
      <span>
        {$t('Track your growth by taking a look at the past month of your stream (data provided from Twitch)')}
      </span>
      <div className={styles.streamPulseContainer}>
        {STATS_TO_MAP().map(stat => (<div className={styles.card} key={stat.value}>
...
function ResourceFooter(p) {
    return (<div className={styles.streamerResources}>
...
function GrowthTips() {
    ...
    return (<div className={styles.growthTipsContainer}>
      <h2>{$t('Growth Tips')}</h2>
      <Scrollable isResizable={false} style={{ height: '100%' }}>
        {tips.current.map(tip => (<div className={styles.card} key={tip.title}>
