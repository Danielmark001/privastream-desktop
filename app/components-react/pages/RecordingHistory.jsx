...
export default function RecordingHistoryPage(p) {
    ...
    return (<RecordingHistoryCtx.Provider value={controller}>
...
export function RecordingHistory(p) {
    ...
    function UploadActions(p) {
        return (<span className={styles.actionGroup}>
        {uploadOptions
                .map(option => {
                ...
                return (<span className={styles.action} key={option.value} style={{
    ...
    return (<div className={cx(styles.container, p.className)}>
      ...
      <div className={styles.recordingsContainer} id="recordingHistory">
        <Scrollable style={{ height: '100%' }}>
          {recordings.map(recording => (<div className={styles.recording} key={recording.timestamp}>
              <span style={{ marginRight: '8px' }}>{formattedTimestamp(recording.timestamp)}</span>
              <Tooltip title={$t('Show in folder')}>
                <span data-test="filename" onClick={() => showFile(recording.filename)} className={styles.filename}>
                  {recording.filename}
                </span>
              </Tooltip>
...
function EditModal() {
    ...
    return (<div className={styles.modalBackdrop}>
      <ModalLayout hideFooter wrapperStyle={{
            width: '750px',
            height: '320px',
        }} bodyStyle={{
            width: '100%',
        ...
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}>
            {editOptions.map(editOpt => (<div className={styles.editCell} key={editOpt.value} onClick={() => editFile(editOpt.value)}>
...
//# sourceMappingURL=RecordingHistory.jsx.map