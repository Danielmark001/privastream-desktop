...
export default function TitleBar(props) {
    ...
    return (<>
      <div className={cx(styles.titlebar, theme, {
            [styles['titlebar-mac']]: isMac,
            [styles.titlebarError]: errorState,
        })} data-name="title-bar">
        {!primeTheme && !isMac && (<img className={styles.titlebarIcon} src={require('../../../media/images/icon.ico')}/>)}
        {primeTheme && !isMac && <KevinSvg className={styles.titlebarIcon}/>}
        <div className={styles.titlebarTitle} onDoubleClick={maximize}>
          {title}
        </div>
        {!isMac && (<div className={styles.titlebarActions}>
            {isDev && (<i className={cx('fas fa-sync', styles.titlebarAction)} onClick={() => window.location.reload()}/>)}
            <i className={cx('icon-subtract', styles.titlebarAction)} onClick={minimize}/>
            {isMaximizable && (<i className={cx('icon-expand-1', styles.titlebarAction)} onClick={maximize}/>)}
