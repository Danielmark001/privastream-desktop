...
export default function PerformanceMetrics(props) {
    ...
    return (<div className={cx(styles.performanceMetrics, 'performance-metrics', 'flex flex--center', props.className)}>
      {shownCells.map((attribute) => {
            const data = metadata[attribute];
            return (<Tooltip placement="bottom" title={pinTooltip(data.label)} key={attribute}>
            <span className={cx(styles.performanceMetricWrapper, classForStat(attribute), 'performance-metric-wrapper')} onClick={() => updatePinnedStats(attribute, !pinnedStats[attribute])}>
              ...
              <span className={styles.performanceMetric}>
                ...
                {showLabel(attribute) && (<span className={styles.performanceMetricLabel}> {data.label}</span>)}
              </span>
            </span>
          </Tooltip>);
        })}
...
//# sourceMappingURL=PerformanceMetrics.jsx.map