import React from 'react';
import { Tooltip } from 'antd';
import cx from 'classnames';
import { useVuex } from '../hooks';
import { Services } from '../service-provider';
import styles from './PerformanceMetrics.m.less';
import { $t } from '../../services/i18n';
import { useRealmObject } from 'components-react/hooks/realm';
export default function PerformanceMetrics(props) {
    const { CustomizationService, PerformanceService } = Services;
    const pinnedStats = useRealmObject(CustomizationService.state.pinnedStatistics);
    const v = useVuex(() => ({
        cpuPercent: PerformanceService.views.cpuPercent,
        frameRate: PerformanceService.views.frameRate,
        droppedFrames: PerformanceService.views.droppedFrames,
        percentDropped: PerformanceService.views.percentDropped,
        bandwidth: PerformanceService.views.bandwidth,
    }), false);
    function showAttribute(attribute) {
        return props.mode === 'full' || pinnedStats[attribute];
    }
    function pinTooltip(stat) {
        return props.mode === 'full' ? $t('Click to add %{stat} info to your footer', { stat }) : '';
    }
    function classForStat(stat) {
        if (props.mode === 'limited')
            return '';
        return `clickable ${pinnedStats[stat] ? 'active' : ''}`;
    }
    function updatePinnedStats(key, value) {
        if (props.mode === 'limited')
            return;
        CustomizationService.actions.setSettings({ pinnedStatistics: { [key]: value } });
    }
    const metadata = {
        cpu: { value: `${v.cpuPercent}%`, label: $t('CPU'), icon: 'icon-cpu' },
        fps: { value: v.frameRate, label: 'FPS', icon: 'icon-fps' },
        droppedFrames: {
            value: `${v.droppedFrames} (${v.percentDropped}%)`,
            label: $t('Dropped Frames'),
            icon: 'icon-dropped-frames',
        },
        bandwidth: { value: v.bandwidth, label: 'kb/s', icon: 'icon-bitrate' },
    };
    const shownCells = [
        'cpu',
        'fps',
        'droppedFrames',
        'bandwidth',
    ].filter((val) => showAttribute(val));
    function showLabel(attribute) {
        if (attribute !== 'droppedFrames')
            return true;
        return props.mode === 'full';
    }
    return (React.createElement("div", { className: cx(styles.performanceMetrics, 'performance-metrics', 'flex flex--center', props.className) }, shownCells.map((attribute) => {
        const data = metadata[attribute];
        return (React.createElement(Tooltip, { placement: "bottom", title: pinTooltip(data.label), key: attribute },
            React.createElement("span", { className: cx(styles.performanceMetricWrapper, classForStat(attribute), 'performance-metric-wrapper'), onClick: () => updatePinnedStats(attribute, !pinnedStats[attribute]) },
                React.createElement("i", { className: cx(styles.performanceMetricIcon, data.icon) }),
                React.createElement("span", { className: styles.performanceMetric },
                    React.createElement("span", { className: styles.performanceMetricValue, role: `metric-${attribute}` }, data.value),
                    showLabel(attribute) && (React.createElement("span", { className: styles.performanceMetricLabel },
                        " ",
                        data.label))))));
    })));
}
//# sourceMappingURL=PerformanceMetrics.js.map