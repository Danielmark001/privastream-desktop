...
function SceneSelector() {
    ...
    const DropdownMenu = (<div className={cx(styles.dropdownContainer, 'react')}>
      ...
      <Scrollable style={{ height: 'calc(100% - 60px)' }}>
        {collections.map(collection => (<div key={collection.id} onClick={() => loadCollection(collection.id)} className={cx(styles.dropdownItem, {
                [styles.osMismatch]: getOS() !== collection.operatingSystem,
            })} data-name={collection.name}>
            <i className={cx('fab', collection.operatingSystem === 'win32' ? 'fa-windows' : 'fa-apple')}/>
            {collection.name}
          </div>))}
      </Scrollable>
    </div>);
    return (<>
      <div className={styles.topContainer} id="sceneSelector">
        <Dropdown overlay={DropdownMenu} trigger={['click']} getPopupContainer={() => document.getElementById('sceneSelector')} visible={showDropdown} onVisibleChange={setShowDropdown} placement="bottomLeft">
          <span className={styles.activeSceneContainer} data-name="SceneSelectorDropdown">
            <DownOutlined style={{ marginRight: '4px' }}/>
            <span className={styles.activeScene}>{activeCollection === null || activeCollection === void 0 ? void 0 : activeCollection.name}</span>
          </span>
        </Dropdown>
        ...
        {v.showDualOutput && (<DisplayToggle name="editor-displays" className={styles.editorDisplayToggle}/>)}
      ...
      <Scrollable style={{ height: '100%' }} className={styles.scenesContainer}>
        <Tree draggable treeData={scenes} onDrop={handleSort} onSelect={makeActive} onRightClick={showContextMenu} selectedKeys={[activeSceneId]}/>
      </Scrollable>
      <HelpTip title={$t('Scene Collections')} dismissableKey={EDismissable.SceneCollectionsHelpTip} position={{ top: '-8px', left: '102px' }}>
        <div>
...
function TreeNode(p) {
    return (<div className={styles.sourceTitleContainer} data-name={p.scene.name} data-role="scene">
...
export function SceneSelectorElement() {
    ...
    return (<div ref={containerRef} data-name="SceneSelector" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {renderElement()}
    </div>);
}
SceneSelectorElement.mins = mins;
//# sourceMappingURL=SceneSelector.jsx.map