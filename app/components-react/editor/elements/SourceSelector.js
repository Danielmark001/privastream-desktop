    ...
    getTreeData(nodeData) {
        const getTreeNodes = (sceneNodes) => {
            return sceneNodes.map(sceneNode => {
                ...
                return {
                    title: (React.createElement(TreeNode, { title: sceneNode.title, id: sceneNode.id, sceneId: sceneNode.sceneId, isVisible: sceneNode.isVisible, isLocked: sceneNode.isLocked, canShowActions: sceneNode.canShowActions, toggleVisibility: () => this.toggleVisibility(sceneNode.id, sceneNode === null || sceneNode === void 0 ? void 0 : sceneNode.toggleAll), toggleLock: () => this.toggleLock(sceneNode.id), selectiveRecordingEnabled: this.selectiveRecordingEnabled, isStreamVisible: sceneNode.isStreamVisible, isRecordingVisible: sceneNode.isRecordingVisible, isGuestCamActive: sceneNode.isGuestCamActive, isDualOutputActive: sceneNode.isDualOutputActive, hasNodeMap: this.hasNodeMap, cycleSelectiveRecording: () => this.cycleSelectiveRecording(sceneNode.id), ref: this.nodeRefs[sceneNode.id], onDoubleClick: () => this.sourceProperties(sceneNode.id), removeSource: () => this.removeItems(sceneNode.id), sourceProperties: () => this.sourceProperties(sceneNode.id) })),
                    isLeaf: !children,
                    key: sceneNode.id,
                    switcherIcon: React.createElement("i", { className: sceneNode.icon }),
                    children,
                };
            });
        };
...
function SourceSelector() {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(StudioControls, null),
        React.createElement(ItemsTree, null),
        ctrl.nodeData.some(node => node.isFolder) && (React.createElement(HelpTip, { title: $t('Folder Expansion'), dismissableKey: EDismissable.SourceSelectorFolders, position: { top: '-8px', left: '102px' } },
            React.createElement(Translate, { message: $t('Wondering how to expand your folders? Just click on the <icon></icon> icon') },
                React.createElement("i", { slot: "icon", className: "fa fa-folder" })))),
        React.createElement(AuthModal, { prompt: $t('Please log in to enable dual output. Would you like to log in now?'), showModal: showModal, handleShowModal: ctrl.handleShowModal, handleAuth: ctrl.handleAuth })));
}
function StudioControls() {
    ...
    return (React.createElement("div", { className: styles.topContainer, "data-name": "sourcesControls" },
        React.createElement("div", { className: styles.activeSceneContainer },
            React.createElement(Tooltip, { title: sourcesTooltip, placement: "bottomLeft" },
                React.createElement("span", { className: styles.sourcesHeader }, $t('Sources')))),
        React.createElement(Tooltip, { title: addSourceTooltip, placement: "bottomLeft" },
            React.createElement("i", { className: "icon-add-circle icon-button icon-button--lg", onClick: () => ctrl.addSource() })),
        React.createElement(Tooltip, { title: ctrl.dualOutputTitle, placement: "bottomRight" },
            React.createElement("i", { className: cx('icon-dual-output icon-button icon-button--lg', {
        ...
        React.createElement(Tooltip, { title: $t('Toggle Selective Recording'), placement: "bottomRight" },
            React.createElement("i", { className: cx('icon-smart-record icon-button icon-button--lg', {
                    active: selectiveRecordingEnabled,
                    disabled: selectiveRecordingLocked,
                }), onClick: () => ctrl.toggleSelectiveRecording() })),
        React.createElement(Tooltip, { title: addGroupTooltip, placement: "bottomRight" },
            React.createElement("i", { className: "icon-add-folder icon-button icon-button--lg", onClick: () => ctrl.addFolder() }))));
}
function ItemsTree() {
    const ctrl = useController(SourceSelectorCtx);
    ...
    return (React.createElement("div", { style: { height: 'calc(100% - 33px)' }, onMouseEnter: (e) => setShowTreeMask(e.buttons !== 0), onMouseUp: () => setShowTreeMask(false), onMouseLeave: () => setShowTreeMask(true) },
        React.createElement(Scrollable, { className: cx(styles.scenesContainer, styles.sourcesContainer), onContextMenu: (e) => ctrl.showContextMenu('', e) },
            showTreeMask && React.createElement("div", { className: styles.treeMask, "data-name": "treeMask" }),
            React.createElement(Tree, { selectedKeys: selectionItemIds, expandedKeys: expandedFoldersIds, onSelect: (selectedKeys, info) => ctrl.makeActive(info), onExpand: (selectedKeys, info) => ctrl.toggleFolder(info.node.key), onRightClick: info => ctrl.showContextMenu(info.node.key, info.event), onDrop: (info) => ctrl.handleSort(info), treeData: treeData, draggable: true, multiple: true, blockNode: true, showIcon: true }))));
}
const TreeNode = React.forwardRef((p, ref) => {
    ...
    return (React.createElement("div", { className: styles.sourceTitleContainer, "data-name": p.title, "data-role": "source", ref: ref, onDoubleClick: p.onDoubleClick },
        React.createElement("span", { className: styles.sourceTitle }, p.title),
        p.canShowActions && (React.createElement(React.Fragment, null,
            p.isGuestCamActive && React.createElement("i", { className: "fa fa-signal" }),
            p.isDualOutputActive && p.hasNodeMap && (React.createElement(DualOutputSourceSelector, { nodeId: p.id, sceneId: p === null || p === void 0 ? void 0 : p.sceneId })),
            p.selectiveRecordingEnabled && (React.createElement(Tooltip, { title: selectiveRecordingMetadata().tooltip, placement: "left" },
                React.createElement("i", { className: cx(selectiveRecordingMetadata().icon, { disabled: p.isLocked }), onClick: p.cycleSelectiveRecording }))),
            React.createElement(Tooltip, { title: $t('Lock/Unlock Source'), placement: "left", visible: ['icon-lock', 'icon-unlock'].includes(hoveredIcon) },
                React.createElement("i", { onClick: p.toggleLock, className: p.isLocked ? 'icon-lock' : 'icon-unlock', onMouseEnter: () => setHoveredIcon(p.isLocked ? 'icon-lock' : 'icon-unlock'), onMouseLeave: () => setHoveredIcon('') })),
            React.createElement(Tooltip, { title: $t('Hide/Unhide'), placement: "left", visible: ['icon-view', 'icon-hide'].includes(hoveredIcon) },
                React.createElement("i", { onClick: p.toggleVisibility, className: p.isVisible ? 'icon-view' : 'icon-hide', onMouseEnter: () => setHoveredIcon(p.isVisible ? 'icon-view' : 'icon-hide'), onMouseLeave: () => setHoveredIcon('') })))),
        React.createElement(Tooltip, { title: $t('Remove Sources from your Scene.'), placement: "left", visible: hoveredIcon === 'icon-trash' },
            React.createElement("i", { onClick: p.removeSource, className: "icon-trash", onMouseEnter: () => setHoveredIcon('icon-trash'), onMouseLeave: () => setHoveredIcon('') })),
        React.createElement(Tooltip, { title: $t('Open the Source Properties.'), placement: "left", visible: hoveredIcon === 'icon-settings' },
            React.createElement("i", { onClick: p.sourceProperties, className: "icon-settings", onMouseEnter: () => setHoveredIcon('icon-settings'), onMouseLeave: () => setHoveredIcon('') }))));
});
const mins = { x: 200, y: 120 };
export function SourceSelectorElement() {
    const containerRef = useRef(null);
    const { renderElement } = useBaseElement(React.createElement(SourceSelector, null), mins, containerRef.current);
    const controller = useMemo(() => new SourceSelectorController(), []);
    return (React.createElement("div", { ref: containerRef, "data-name": "SourceSelector", style: { height: '100%', display: 'flex', flexDirection: 'column' } },
        React.createElement(SourceSelectorCtx.Provider, { value: controller }, renderElement())));
}
