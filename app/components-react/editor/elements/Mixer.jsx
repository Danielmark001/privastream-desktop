...
export function Mixer() {
    ...
    function Element() {
        ...
        <div className="studio-controls-top">
          <Tooltip title={$t('Monitor audio levels. If the bars are moving you are outputting audio.')} placement="bottom">
            <h2 className="studio-controls__label">{$t('Mixer')}</h2>
          </Tooltip>
          <Tooltip title={$t('Open advanced audio settings')} placement="left">
            <i className="icon-settings icon-button" role="show-advanced-audio" onClick={showAdvancedSettings}/>
          </Tooltip>
        </div>
        <Scrollable className="studio-controls-selector mixer-panel" style={{ height: 'calc(100% - 32px)' }}>
          <div style={{ position: 'relative' }} onContextMenu={handleRightClick}>
            {audioSourceIds.length !== 0 && !performanceMode && <GLVolmeters />}
            {audioSourceIds.map(sourceId => (<MixerItem key={sourceId} audioSourceId={sourceId} volmetersEnabled={needToRenderVolmeters}/>))}
          </div>
        </Scrollable>
      </>);
    }
...
Mixer.mins = mins;
//# sourceMappingURL=Mixer.jsx.map