...
export default function NavTools(p) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Menu, { key: ENavName.BottomNav, forceSubMenuRender: true, mode: "inline", className: cx(styles.bottomNav, !isOpen && styles.closed, isOpen && styles.open), defaultOpenKeys: openMenuItems && openMenuItems, getPopupContainer: triggerNode => triggerNode }, menuItems.map((menuItem) => {
            if (isDevMode && menuItem.key === EMenuItemKey.DevTools) {
                return React.createElement(NavToolsItem, { key: menuItem.key, menuItem: menuItem, onClick: openDevTools });
            }
            else if (!isPrime && menuItem.key === EMenuItemKey.GetPrime) {
                return (React.createElement(NavToolsItem, { key: menuItem.key, menuItem: menuItem, icon: React.createElement("div", null,
                        React.createElement(Badge, { count: React.createElement("i", { className: cx('icon-pop-out-3', styles.linkBadge) }) },
                            React.createElement(UltraIcon, null))), onClick: upgradeToPrime, className: styles.badgeScale }));
            }
            else if (isLoggedIn && menuItem.key === EMenuItemKey.Dashboard) {
                return (React.createElement(SubMenu, { key: menuItem.key, title: menuTitles(menuItem.key), icon: React.createElement("div", null,
                        React.createElement(Badge, { count: React.createElement("i", { className: cx('icon-pop-out-3', styles.linkBadge) }) },
                            React.createElement("i", { className: cx(menuItem.icon, 'small') }))), onTitleClick: () => {
                        !isOpen && throttledOpenDashboard();
                        expandMenuItem(ENavName.BottomNav, menuItem.key);
                    } },
                    React.createElement(DashboardSubMenu, { subMenuItems: menuItem === null || menuItem === void 0 ? void 0 : menuItem.subMenuItems, throttledOpenDashboard: throttledOpenDashboard, openSettingsWindow: openSettingsWindow })));
            }
            else if (menuItem.key === EMenuItemKey.GetHelp) {
                return (React.createElement(NavToolsItem, { key: menuItem.key, menuItem: menuItem, icon: React.createElement("div", null,
                        React.createElement(Badge, { count: React.createElement("i", { className: cx('icon-pop-out-3', styles.linkBadge) }) },
                            React.createElement("i", { className: menuItem === null || menuItem === void 0 ? void 0 : menuItem.icon }))), onClick: () => openHelp() }));
            }
            else if (showAiTab && menuItem.key === EMenuItemKey.AI) {
                return (React.createElement(NavToolsItem, { key: menuItem.key, menuItem: menuItem, className: cx({ [styles.vision]: p.isVisionRunning }), onClick: () => openSettingsWindow(ESettingsCategory.AI) }));
            }
            else if (menuItem.key === EMenuItemKey.Settings) {
                return (React.createElement(NavToolsItem, { key: menuItem.key, menuItem: menuItem, onClick: () => openSettingsWindow() }));
            }
            else if (menuItem.key === EMenuItemKey.Login) {
                return (React.createElement(LoginMenuItem, { key: menuItem.key, menuItem: menuItem, handleAuth: handleAuth, handleShowModal: handleShowModal }));
            }
        })),
        React.createElement(AuthModal, { title: $t('Confirm'), prompt: confirmMsg, showModal: showModal, handleAuth: handleAuth, handleShowModal: handleShowModal })));
}
function NavToolsItem(p) {
    ...
    return (React.createElement(MenuItem, { title: title, icon: icon !== null && icon !== void 0 ? icon : React.createElement("i", { className: menuItem === null || menuItem === void 0 ? void 0 : menuItem.icon }), className: className, onClick: onClick }, title));
}
function DashboardSubMenu(p) {
    ...
    return (React.createElement(React.Fragment, null, subMenuItems.map((subMenuItem) => (React.createElement(MenuItem, { key: subMenuItem.key, title: menuTitles(subMenuItem.key), onClick: () => handleNavigation(subMenuItem === null || subMenuItem === void 0 ? void 0 : subMenuItem.type) }, menuTitles(subMenuItem.key))))));
}
function LoginMenuItem(p) {
    ...
    return (React.createElement(MenuItem, { "data-testid": "nav-auth", title: !isLoggedIn ? menuTitles(menuItem.key) : $t('Log Out'), className: cx(styles.login, !isOpen && styles.loginClosed), icon: !isOpen && React.createElement("i", { className: "icon-user" }), onClick: () => (isLoggedIn ? handleShowModal(true) : handleAuth()) }, !isLoggedIn ? (React.createElement("span", { className: styles.loggedOut }, menuTitles(menuItem.key))) : (isOpen && React.createElement(PlatformIndicator, { platform: platform }))));
}
