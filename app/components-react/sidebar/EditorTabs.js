...
export default function EditorTabs(p) {
    ...
    return type === 'root' ? (React.createElement(React.Fragment, null, rootTabs.map(tab => {
        var _a;
        return (React.createElement(MenuItem, { key: tab.key, className: cx(!isOpen && styles.closed, (currentMenuItem === EMenuItemKey.Editor ||
                currentMenuItem === tab.key ||
                currentMenuItem === `sub-${tab.key}`) &&
                styles.active), title: (_a = tab.title) !== null && _a !== void 0 ? _a : defaultTitle, icon: React.createElement("i", { className: tab.icon }), onClick: () => navigateToStudioTab(tab.target, tab.trackingTarget, tab.key) }, tab.title));
    }))) : (React.createElement(React.Fragment, null, studioTabs.map(tab => {
        var _a, _b;
        return (React.createElement(MenuItem, { key: `sub-${tab.key}`, className: cx((currentMenuItem === tab.key || currentMenuItem === `sub-${tab.key}`) && styles.active), title: (_a = tab === null || tab === void 0 ? void 0 : tab.title) !== null && _a !== void 0 ? _a : defaultTitle, icon: React.createElement("i", { className: tab.icon }), onClick: () => navigateToStudioTab(tab.target, tab.trackingTarget, `sub-${tab.key}`), type: "submenu" }, (_b = tab === null || tab === void 0 ? void 0 : tab.title) !== null && _b !== void 0 ? _b : defaultTitle));
    })));
}
