...
export function SponsorBanner() {
    ...
    return (React.createElement(WidgetLayout, null,
        React.createElement(Menu, { onClick: e => setSelectedTab(e.key), selectedKeys: [selectedTab] },
            ...
            positions.map(number => (React.createElement(Menu.Item, { key: number }, $t('Image Set %{number}', { number })))),
            React.createElement(Menu.Item, { key: "visual" }, $t('Visual Settings'))),
        React.createElement(Form, null,
            !isLoading && selectedTab === 'general' && (React.createElement(FormFactory, { metadata: generalMeta, values: formSettings, onChange: updateSetting })),
            !isLoading && ['1', '2'].includes(selectedTab) && (React.createElement(ImageSection, { key: selectedTab, placement: selectedTab, values: settings, updateSetting: updateSetting })),
            !isLoading && selectedTab === 'visual' && (React.createElement(FormFactory, { metadata: visualMeta, values: formSettings, onChange: updateSetting })))));
}
function ImageSection(p) {
    ...
    return (React.createElement(React.Fragment, null,
        images.map((image, i) => (React.createElement("div", { style: { position: 'relative' }, key: `${image.href}${i}` },
            React.createElement(MediaUrlInput, { value: image.href, onChange: val => handleImageChange('href', val, image.href), nowrap: true }),
            ...
            React.createElement(NumberInput, { label: $t('Image Duration (seconds)'), value: image.duration, onChange: val => handleImageChange('duration', val, image.href) })))),
        React.createElement(Button, { className: "button button--default", style: { marginBottom: '16px' }, onClick: addImage }, $t('Add Image'))));
}
...
//# sourceMappingURL=SponsorBanner.js.map