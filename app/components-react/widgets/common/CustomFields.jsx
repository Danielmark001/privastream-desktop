...
export function CustomFields() {
    ...
    return (<Form layout="horizontal">
      {fieldsProps.map(props => (<CustomField {...props} key={props.name}/>))}
    </Form>);
}
function CustomField(p) {
    ...
    switch (p.type) {
        ...
        case 'dropdown':
            return (<ListInput {...commonProps} options={Object.keys(p.options).map(key => ({ value: key, label: p.options[key] }))}/>);
        case 'sound-input':
...
//# sourceMappingURL=CustomFields.jsx.map