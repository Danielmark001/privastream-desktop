...
export function CustomCodeWindow() {
    ...
    return (<ModalLayout footer={<EditorFooter />}>
      {isLoading && <Spin spinning={true}/>}

      
      {!isLoading && (<Tabs activeKey={selectedTab} onChange={selectTab}>
          {tabs.map(tab => (<TabPane tab={tab.label} key={tab.key}>
              <Editor />
            </TabPane>))}
        </Tabs>)}
    </ModalLayout>);
...
function Editor() {
    ...
    return (<>
      ...
      <Alert style={{ marginTop: '7px' }} message={$t('Saving custom code can have potential security risks, make sure you trust the code you are about to apply.')} type="warning" showIcon/>
    </>);
}
function JsonEditor() {
    ...
    return (<>
      <ButtonGroup>
        ...
        {code && (<Button danger onClick={removeCustomFields}>
    ...
    return (<Collapse bordered={false}>
      <Collapse.Panel header={$t('Custom Code')} key={1}>
        <Form layout="horizontal">
          <SwitchInput label={$t('Enable Custom Code')} value={isCustomCodeEnabled} onChange={custom_enabled => updateCustomCode({ custom_enabled })}/>
          {isCustomCodeEnabled && (<ButtonGroup>
...
//# sourceMappingURL=CustomCode.jsx.map