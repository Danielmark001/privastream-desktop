...
export function AlertBox() {
    ...
    return (<WidgetLayout layout={layout}>
      <TabsList />
      <TabContent />
    </WidgetLayout>);
}
...
function TabContent() {
    ...
    return selectedTab === 'general' ? (<GeneralSettings />) : (<VariationSettings type={selectedTab}/>);
}
function GeneralSettings() {
    ...
    return (<Form layout={'horizontal'}>
      <SliderInput label={$t('Global Alert Delay')} {...bind.alert_delay} step={1000} min={0} max={30000} tipFormatter={(ms) => `${ms / 1000}s`} debounce={500}/>

      <Info message={$t('Looking for the old AlertBox settings?')} onClick={switchToLegacyAlertbox}/>
...
function AlertsList() {
    ...
    return (<Menu onClick={onMenuClickHandler} selectedKeys={[selectedTab]} theme={'dark'}>
      {alerts.map(alertEvent => (<Menu.Item key={alertEvent.type}>
          
          <CheckboxInput value={enabledAlerts.includes(alertEvent.type)} onChange={val => setEnabled(alertEvent.type, val)} style={{ display: 'inline-block' }}/>

          
          {alertEvent.name}
          {alertEvent.tooltip && (<Tooltip placement="rightBottom" title={<span>
                  {alertEvent.tooltip}
                  {alertEvent.tooltipLink && (<ButtonGroup>
          ...
          <Tooltip title={$t('Play Alert')} placement="left">
            ...
            }} type={'text'} style={{ position: 'absolute', right: '16px', top: '8px' }} icon={<CaretRightOutlined style={{ fontSize: '36px', color: 'white' }}/>}/>
          </Tooltip>
        </Menu.Item>))}
    </Menu>);
}
...
function CommonAlertSettings(p) {
    ...
    return (<div key={selectedTab} ref={containerRef}>
...
function DonationSettings() {
    ...
    return (<>
      ...
      <Info message={$t('Customize your tip page where viewers can send you donations')} onClick={openTipPageSettings}/>
    </>);
}
...
function Info(p) {
    return (<Alert message={<span>
        ...
        </span>} type="info" showIcon style={{ marginBottom: '16px' }}/>);
}
function AdvancedSettingsPanel() {
    ...
    return (<Collapse bordered={false} style={{ marginBottom: '8px' }}>
      <Collapse.Panel header={$t('Advanced')} key={1}>
        ...
        <InputWrapper label={$t('Alert Moderation delay')}>
          {!isUnlimitedModerationDelay && (<SliderInput {...bind.moderation_delay} min={0} debounce={500} nowrap/>)}
          <CheckboxInput label={$t('Use unlimited delay')} value={isUnlimitedModerationDelay} onChange={switchUnlimitedModeration} tooltip={$t('This applies to all alerts. If enabled all alerts need to be approved manually.')}/>
        </InputWrapper>
      </Collapse.Panel>
    </Collapse>);
}
//# sourceMappingURL=AlertBox.jsx.map