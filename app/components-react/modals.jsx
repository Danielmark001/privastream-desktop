...
export function promptAsync(p, value = '') {
    ...
    return new Promise(resolve => {
        ...
        const dialog = Modal.info(Object.assign(Object.assign({ centered: true, icon: null }, modalProps), { content: (<DefaultPromptForm values={formValues} onValuesChange={onValuesChange} onFinish={onFinish} submitEmitter={submitEmitter}/>), afterClose: () => {
                WindowsService.updateStyleBlockers(Utils.getWindowId(), false);
            }, onOk: () => {
                submitEmitter.next();
                return true;
...
export function promptAction(p) {
    return alertAsync({
        ...
        modalRender: node => {
            var _a, _b, _c;
            return (<ModalLayout footer={<Form layout={'inline'} className={styles.actionModalFooter}>
            {!p.cancelBtnPosition ||
                        (p.cancelBtnPosition && p.cancelBtnPosition === 'left' && (<Button onClick={() => submit(p === null || p === void 0 ? void 0 : p.cancelFn)}>{(_a = p.cancelBtnText) !== null && _a !== void 0 ? _a : $t('Skip')}</Button>))}
            {p.secondaryActionFn && p.secondaryActionText && (<Button onClick={() => submit(p === null || p === void 0 ? void 0 : p.secondaryActionFn)}>{p.secondaryActionText}</Button>)}
            <Button type={(_b = p === null || p === void 0 ? void 0 : p.btnType) !== null && _b !== void 0 ? _b : 'primary'} onClick={() => submit(p === null || p === void 0 ? void 0 : p.fn)}>
              {p.btnText}
            </Button>
            {p.cancelBtnPosition && p.cancelBtnPosition === 'right' && (<Button onClick={() => submit(p === null || p === void 0 ? void 0 : p.cancelFn)}>{(_c = p.cancelBtnText) !== null && _c !== void 0 ? _c : $t('Skip')}</Button>)}
          </Form>}>
        {node}
      </ModalLayout>);
        },
...
export function DefaultPromptForm(p) {
    ...
    return (<Form name="prompt" form={form}>
...
//# sourceMappingURL=modals.jsx.map