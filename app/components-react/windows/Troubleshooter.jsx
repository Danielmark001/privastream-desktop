...
export default function Troubleshooter() {
    ...
    const metadata = {
        ...
        HIGH_CPU_USAGE: {
            ...
            actionControls: (<button className="button button--action" onClick={enablePerformanceMode}>
    ...
    function ModalFooter() {
        return (<>
        {(issue === null || issue === void 0 ? void 0 : issue.code) === 'FRAMES_DROPPED' && <StartStreamingButton />}
    ...
    return (<ModalLayout footer={<ModalFooter />} scrollable>
      <h4>
      ...
      <p style={{ marginBottom: '16px' }}>
        {renderedData.title} {mom(issue === null || issue === void 0 ? void 0 : issue.date)}.<br />
      </p>
      <h4>{$t('What does this mean?')}</h4>
      <p style={{ marginBottom: '16px' }}>{renderedData.meaning}</p>
      <h4>{$t('What can I do?')}</h4>
      <ul style={{ marginBottom: '16px' }}>
        {renderedData.actionItems.map(item => (<li key={item}>{item}</li>))}
      </ul>
      {renderedData.actionControls}
    </ModalLayout>);
}
//# sourceMappingURL=Troubleshooter.jsx.map