...
export function VideoSettings() {
    ...
    return (<>
      <div className={styles.videoSettingsHeader}>
        <h2>{$t('Video')}</h2>
        <DualOutputToggle value={showDualOutputSettings} onChange={toggleDualOutput} disabled={cantEditFields} placement="bottomRight" lightShadow/>
      </div>
      {showDualOutputSettings && <Tabs onChange={setDisplay}/>}

      <div className={styles.formSection}>
        <FormFactory values={values} metadata={metadata} onChange={onChange} formOptions={{ layout: 'vertical' }} name="video-settings"/>
      </div>
      <AuthModal id="login-modal" prompt={$t('Please log in to enable dual output. Would you like to log in now?')} showModal={showModal} handleShowModal={handleShowModal} handleAuth={handleAuth}/>
    </>);
}
