...
function SupportLinks() {
    ...
    return (<ObsSettingsSection title={$t('Support Links')}>
...
function DiagnosticReport() {
    ...
    function uploadReport() {
        ...
        DiagnosticsService.actions.return
            ...
            alertAsync({
              ...
              <TextInput readOnly style={{ marginTop: 20, marginLeft: -10 }} value={r.report_code} addonAfter={<Button onClick={() => remote.clipboard.writeText(r.report_code)}>
                    {$t('Copy')}
                  </Button>}/>
            </div>),
            });
    ...
    return (<ObsSettingsSection title={$t('Diagnostic Report')}>
      ...
      <Button style={{ margin: '20px 0' }} onClick={uploadReport} disabled={uploading}>
        <i className={cx('fa', { 'fa-upload': !uploading, 'fa-spinner fa-pulse': uploading })} style={{ marginRight: 8 }}/>
...
function CacheSettings() {
    ...
    return (<ObsSettingsSection title={$t('Cache Directory')}>
      <p>
...
function CrashReporting() {
    ...
    return (<ObsSettingsSection title={$t('Crash Reporting')}>
      <CheckboxInput name="enable_dump_upload" label={$t('Enable reporting additional information on a crash (requires restart)')} value={enableCrashDumps} onChange={val => CustomizationService.actions.setSettings({ enableCrashDumps: val })}/>
    </ObsSettingsSection>);
}
