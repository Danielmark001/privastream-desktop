...
function Header({ title }) {
    return (React.createElement("h2", { className: "section-title section-title--dropdown", style: { display: 'inline-block', verticalAlign: '-2px' } }, title));
}
export default function HotkeyGroup(props) {
    ...
    const header = React.createElement(Header, Object.assign({}, headerProps));
    const hotkeyContent = useMemo(() => (React.createElement("div", { className: cx({ 'section-content--opened': !!title }, 'section-content') }, renderedHotKeys.map(hotkey => (React.createElement("div", { key: getHotkeyUniqueId(hotkey), className: hasSceneHotkeys ? 'scene-hotkey' : undefined },
        React.createElement(Hotkey, { hotkey: hotkey })))))), [renderedHotKeys, display]);
    return (React.createElement("div", { className: "section" }, !isCollapsible ? (hotkeyContent) : (React.createElement(Collapse, { onChange: () => setExpanded(!expanded), expandIcon: ({ isActive }) => (React.createElement("i", { className: cx('fa', 'section-title-icon', {
                'fa-minus': isActive,
                'fa-plus': !isActive,
            }) })) },
        React.createElement(Panel, { header: header, key: "1" },
            isDualOutputScene && showTabs && React.createElement(Tabs, { onChange: setDisplay }),
            isDualOutputMode &&
                !showTabs &&
                $t('This scene has not been converted for Dual Output. Please make the scene active to add vertical sources.'),
            hotkeyContent)))));
}
//# sourceMappingURL=HotkeyGroup.js.map