...
export function GameOverlay() {
    ...
    function WindowEnableToggles() {
        ...
        return (<div>
        {windows.map(win => (<React.Fragment key={titles[win]}>
            <SwitchInput label={titles[win]} value={windowProperties[win].enabled} onInput={() => GameOverlayService.actions.toggleWindowEnabled(win)}/>
          </React.Fragment>))}
      </div>);
    }
    function ExtraOptions() {
        return (<div>
        <SwitchInput value={previewMode} onInput={togglePreviewMode} label={$t('Toggle positioning mode')}/>
        <SliderInput value={opacity} onChange={setOpacity} {...sliderMetadata}/>
        <button className="button button--action" onClick={resetPosition} style={{ marginBottom: '16px' }}>
          {$t('Reset Overlay Position')}
        </button>
      </div>);
    }
    return (<div className="section">
      {!enabling && (<div className="section-content">
          <SwitchInput value={enabled} onInput={enableGameOverlay} label={$t('Enable in-game overlay')}/>
          ...
          <div style={{ marginBottom: '16px' }}>
            {$t('The in-game overlay is a new experimental feature that allows you to view chat and events ' +
...
//# sourceMappingURL=GameOverlay.jsx.map