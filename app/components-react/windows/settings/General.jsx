...
function ExtraSettings() {
    ...
    return (<>
      <ObsSettingsSection>
        {isLoggedIn && !isFacebook && !isYoutube && (<CheckboxInput value={updateStreamInfoOnLive} onChange={val => CustomizationService.actions.setUpdateStreamInfoOnLive(val)} label={$t('Confirm stream title and game before going live')} name="stream_info_udpate"/>)}
        <CheckboxInput label={$t('Disable hardware acceleration (requires restart)')} value={disableHA} onChange={disableHardwareAcceleration} name="disable_ha"/>
        <CheckboxInput label={$t('Disable live streaming features (Recording Only mode)')} value={recordingMode} onChange={RecordingModeService.actions.setRecordingMode}/>
      ...
      <ObsSettingsSection>
        <div className="actions">
          ...
          {canRunOptimizer && (<div className="input-container">
...
//# sourceMappingURL=General.jsx.map