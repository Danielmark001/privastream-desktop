...
export function Hotkeys(props) {
    ...
    function renderHotkeyGroup(id, hotkeys, title, isDualOutputScene = false) {
        return (<HotkeyGroup key={id} title={title} hotkeys={hotkeys} isSearch={isSearch} hasSceneHotkeys={hasSceneHotkeys} isDualOutputMode={isDualOutputMode} isDualOutputScene={isDualOutputScene}/>);
    }
    ...
    return (<div>
      {hasGeneralHotkeys && (<>
          {getOS() === OS.Mac && (<div style={{ display: 'flex', alignItems: 'center' }}>
              <h2>{$t('Hotkeys')}</h2>
              <Tooltip title={$t('To use hotkeys on Mac, go to System Settings > Security > Accessibility and toggle on for Streamlabs Desktop.')} lightShadow wrapperStyle={{ marginBottom: '4px' }} placement="leftTop">
                <i className="icon-information" style={{ padding: '0 0 4px 5px' }}/>
              </Tooltip>
            </div>)}
          <HotkeyGroup hotkeys={generalHotkeys} isSearch={isSearch} title={null} isDualOutputMode={isDualOutputMode}/>
        </>)}
      {hasSceneHotkeys && (<>
          <h2>{$t('Scenes')}</h2>
      ...
      {hasMarkers && (<>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <h2>{$t('Markers')}</h2>
            <a onClick={openMarkersInfoPage} style={{
                fontWeight: 'normal',
          ...
          <HotkeyGroup hotkeys={markerHotkeys} isSearch={isSearch} title={null} isDualOutputMode={isDualOutputMode}/>
        </>)}
    </div>);
}
//# sourceMappingURL=Hotkeys.jsx.map