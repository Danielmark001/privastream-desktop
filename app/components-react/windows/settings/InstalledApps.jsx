...
export function InstalledApps() {
    ...
    return (<div className="section">
      <table className={styles.installedApps} style={{ width: '100%' }}>
        ...
        <tbody>
          {installedApps.map(app => (<tr key={app.id}>
              <td>
                {' '}
              ...
              <td className={cx(styles.buttonContainer, 'button-container--right')}>
                {isEnabled(app.id) && (<button onClick={() => reload(app.id)} className="button button--trans">
                    <i className="icon-reset"></i>
                    {$t('Reload')}
                  </button>)}
                {noUnpackedVersionLoaded(app.id) && (<button onClick={() => toggleEnable(app)} className={cx('button', {
                ...
                {!isEnabled(app.id) && !noUnpackedVersionLoaded(app.id) && (<>
                    ...
                    <Tooltip title={$t('You must unload unpacked version before enabling this app.')} placement="left">
                      <i className="icon-question"/>
                    </Tooltip>
                  </>)}
              </td>
            </tr>))}
          {highlighterVersion !== '' && (<tr key={'Ai Highlighter'}>
              <td>
                <div className={styles.aiHighlighterThumbnail}>
                  <i style={{ margin: 0, fontSize: '20px', color: 'black' }} className="icon-highlighter"></i>
                </div>
              </td>
...
//# sourceMappingURL=InstalledApps.jsx.map