...
export const CommonPlatformFields = InputComponent((rawProps) => {
    ...
    return (<div>
      ...
      {hasCustomCheckbox && (<InputWrapper layout={p.layout}>
          <CheckboxInput name="customEnabled" value={p.value.useCustomFields} onChange={toggleUseCustom} label={title}/>
        </InputWrapper>)}

      <Animate transitionName="slidedown">
        {fieldsAreVisible && (<div>
            
            <TextInput value={fields['title']} name="title" onChange={val => updateCommonField('title', val)} label={titleTooltip ? (<Tooltip title={titleTooltip} placement="right">
                    {$t('Title')}
                    <i className="icon-information" style={{ marginLeft: '5px' }}/>
                  </Tooltip>) : ($t('Title'))} required={true} max={maxCharacters} layout={p.layout} size="large"/>

            
            {hasDescription && (<TextAreaInput value={fields['description']} onChange={val => updateCommonField('description', val)} name="description" label={$t('Description')} required={descriptionIsRequired} layout={p.layout}/>)}
...
//# sourceMappingURL=CommonPlatformFields.jsx.map