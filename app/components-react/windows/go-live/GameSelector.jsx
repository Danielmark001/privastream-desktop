...
export default function GameSelector(p) {
    ...
    return (<ListInput label={label} name={`${p.platform}Game`} value={selectedGameId} extra={p.extra} onChange={p.onChange} placeholder={$t('Start typing to search')} options={games} showSearch onSearch={onSearch} onSelect={(val, opts) => {
            onSelect(opts.labelrender);
        }} filterOption={filterOption} debounce={500} required={isTwitch || isTrovo || isKick} hasImage={isTwitch || isTrovo || isKick} onBeforeSearch={onBeforeSearchHandler} imageSize={platformService.gameImageSize} loading={isSearching} notFoundContent={isSearching ? $t('Searching...') : $t('No matching game(s) found.')} allowClear layout={p.layout} size="large"/>);
}
//# sourceMappingURL=GameSelector.jsx.map