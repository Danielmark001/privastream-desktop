...
export default function GoLiveChecklist(p) {
    ...
    function render() {
        return (React.createElement("div", { className: cx(css.container, p.className, { [css.success]: success }) },
            React.createElement("h1", { className: css.success }, getHeaderText()),
    ...
    function renderCheck(title, state) {
        ...
        switch (state) {
            ...
            case 'pending':
                color = 'orange';
                dot = React.createElement(LoadingOutlined, { spin: false, color: color });
                break;
            case 'done':
                color = 'green';
                dot = React.createElement(CheckCircleOutlined, { color: color });
                break;
            case 'failed':
                color = '#B14334';
                dot = React.createElement(CloseCircleOutlined, { color: color });
                break;
        }
        return (React.createElement(Timeline.Item, { key: title, dot: dot, color: color, className: state === 'done' ? css.done : '' },
            React.createElement("span", null, title)));
    }
    function renderYtWarning() {
        return (React.createElement(MessageLayout, null,
            React.createElement("p", null, $t('Auto-start is disabled for your broadcast. You should manually publish your stream from Youtube Studio')),
            React.createElement("button", { className: "button button--default", onClick: () => Services.YoutubeService.actions.openDashboard() }, $t('Open Youtube Studio'))));
    }
    return render();
}
//# sourceMappingURL=GoLiveChecklist.js.map