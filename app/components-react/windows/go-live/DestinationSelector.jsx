...
export default function DestinationSelector(p) {
    ...
    const options = useMemo(() => {
        const platforms = platformTargets
            ...
            value: platform,
            label: (<>
            <PlatformLogo platform={platform} className={styles.selectorIcon} fontIcon={['tiktok', 'trovo'].includes(platform) ? platform : undefined}/>
    ...
    return (<Select data-test="destination-selector" defaultValue={defaultLabel[0]} className={cx(styles.platformsSelector, { [styles.dualOutputSelector]: isDualOutputMode })} onChange={(option) => {
            if (option.value === 'add' ||
        ...
        }} labelInValue value={defaultLabel[0]} size="large">
      {options.map((option) => {
            var _a;
            return (<Option key={option.value} value={(_a = option.value) !== null && _a !== void 0 ? _a : ''}>
          {option.label}
        </Option>);
        })}
...
//# sourceMappingURL=DestinationSelector.jsx.map