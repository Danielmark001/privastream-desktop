...
export default function RecordingSwitcher(p) {
    ...
    return (<div style={p === null || p === void 0 ? void 0 : p.style} className={cx(p === null || p === void 0 ? void 0 : p.className, styles.recordingSwitcher)}>
      <Tooltip title={$t('AI Highlighter is enabled. Recording will start when stream starts.')} disabled={!v.useAiHighlighter} placement="topRight" lightShadow className={styles.recordingTooltip}>
        <SwitchInput name="recording-toggle" value={recordWhenStartStream} onChange={val => {
            Services.SettingsService.actions.setSettingValue('General', 'RecordWhenStreaming', val);
        }} uncontrolled style={{ marginRight: '10px' }} label={v.isDualOutputMode ? $t('Record Stream in') : $t('Record Stream')} layout="horizontal" checkmark disabled={v.useAiHighlighter}/>
        {v.isDualOutputMode && canRecordVertical && (<>
            <RadioInput name="recording-display" value={recording[0]} options={[
...
//# sourceMappingURL=RecordingSwitcher.jsx.map