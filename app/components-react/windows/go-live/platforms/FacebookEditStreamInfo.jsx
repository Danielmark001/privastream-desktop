...
export default function FacebookEditStreamInfo(p) {
    ...
    setProps(p);
    return (<Form name="facebook-settings">
      {shouldShowPermissionWarn && <PermissionsWarning />}

      <PlatformSettingsLayout layoutMode={p.layoutMode} commonFields={<CommonFields key="common"/>} requiredFields={<RequiredFields key="required"/>} optionalFields={<OptionalFields key="optional"/>} essentialOptionalFields={<Events key="events"/>}/>
    </Form>);
}
function CommonFields() {
    const { settings, updateSettings, layoutMode, layout } = useFacebook();
    return (<CommonPlatformFields key="common" platform="facebook" layoutMode={layoutMode} value={settings} onChange={updateSettings} layout={layout}/>);
}
function RequiredFields() {
    ...
    return (<div key="required">
      {!isUpdateMode && (<>
          {shouldShowDestinationType && (<ListInput label={$t('Facebook Destination')} {...bind.destinationType} hasImage imageSize={{ width: 35, height: 35 }} options={getDestinationOptions()} layout={layout} size="large"/>)}
          {shouldShowPages && (<ListInput {...bind.pageId} required={true} label={$t('Facebook Page')} hasImage imageSize={{ width: 44, height: 44 }} onDropdownVisibleChange={shown => shown && loadPictures('page')} options={pages.map(page => ({
                    value: page.id,
                    label: `${page.name} | ${page.category}`,
                    image: pictures[page.id],
                }))} layout={layout} size="large"/>)}
          {shouldShowGroups && (<>
              <ListInput {...bind.groupId} required={true} label={$t('Facebook Group')} hasImage imageSize={{ width: 44, height: 44 }} options={groups.map(group => ({
                    value: group.id,
                    label: group.name,
                    image: pictures[group.id],
                }))} defaultActiveFirstOption onDropdownVisibleChange={() => loadPictures('group')} extra={<p>
                    {$t('Make sure the Streamlabs app is added to your Group.')}
                    <a onClick={verifyGroup}> {$t('Click here to verify.')}</a>
                  </p>} layout={layout} size="large"/>
            </>)}
        </>)}
    </div>);
}
function OptionalFields() {
    ...
    return (<div key="optional">
      {shouldShowPrivacy && (<ListInput label={$t('Privacy')} value={(_a = settings.privacy) === null || _a === void 0 ? void 0 : _a.value} onChange={setPrivacy} hasImage={true} imageSize={{ width: 24, height: 24 }} options={getPrivacyOptions()} className={css.privacySelector} extra={shouldShowPrivacyWarn && (<Translate message={$t('FBPrivacyWarning')}>
                <a slot="link" onClick={() => remote.shell.openExternal('https://www.facebook.com/settings?tab=business_tools')}/>
              </Translate>)} layout={layout} size="large"/>)}

      {shouldShowGame && (<GameSelector {...bind.game} platform="facebook" extra={shouldShowGamingWarning && (<Translate message={$t('facebookGamingWarning')}>
...
function Events() {
    ...
    return (<div key="events">
      {shouldShowEvents && (<ListInput {...bind.liveVideoId} onChange={onEventChange} label={$t('Scheduled Video')} loading={!scheduledVideosLoaded} allowClear placeholder={$t('Not selected')} options={[
...
function PermissionsWarning() {
    ...
    return (<MessageLayout message={$t('You can stream to your timeline and groups now')} type={'success'}>
      {isPrimary && (<div>
...
//# sourceMappingURL=FacebookEditStreamInfo.jsx.map