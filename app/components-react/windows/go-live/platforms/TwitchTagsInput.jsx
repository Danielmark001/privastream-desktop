...
export function TwitchTagsInput(p) {
    ...
    if (!hasTagsScope) {
        return (<a onClick={() => reauth()} style={{ marginBottom: '8px', display: 'block' }}>
    ...
    return (<TagsInput name="twitchTags" label={p.label} value={tags} max={10} mode="tags" onChange={values => p.onChange && p.onChange(values)} tagRender={(tagProps, tag) => (<Tag {...tagProps} color="#9146FF">
          {tag.label}
        </Tag>)} rules={[{ validator: specialCharacterValidator }]} placeholder={$t('For example: "Speedrunning" or "FirstPlaythrough"')} tokenSeparators={[' ', ',']} dropdownStyle={{ display: 'none' }} layout={p.layout} size="large"/>);
}
//# sourceMappingURL=TwitchTagsInput.jsx.map