...
export function TikTokEditStreamInfo(p) {
    ...
    return (<Form name="tiktok-settings">
      <PlatformSettingsLayout layoutMode={p.layoutMode} commonFields={<CommonPlatformFields key="common" platform="tiktok" layoutMode={p.layoutMode} value={ttSettings} onChange={updateSettings} layout={p.layout}/>} requiredFields={<div key="empty-tiktok"/>}/>
      {approved && (<GameSelector key="optional" platform={'tiktok'} {...bind.game} layout={p.layout}/>)}
      {approved && !controls.disable && (<RadioInput key="audience-ctrl" options={controls.types} defaultValue={controls.audienceType} value={controls.audienceType} label={$t('TikTok Audience')} direction="horizontal" colon {...bind.audienceType} layout={p.layout}/>)}
...
export function TikTokEnterCredentialsFormInfo(p) {
    ...
    return (<>
      <TextInput label={<Tooltip title={$t('Generate with "Locate my Stream Key"')} placement="right">
            {$t('TikTok Server URL')}
            <i className="icon-information" style={{ marginLeft: '5px' }}/>
          </Tooltip>} required {...bind.serverUrl} layout={p.layout} size="large"/>
      <TextInput label={<Tooltip title={$t('Generate with "Locate my Stream Key"')} placement="right">
            {$t('TikTok Stream Key')}
            <i className="icon-information" style={{ marginLeft: '5px' }}/>
          </Tooltip>} required {...bind.streamKey} layout={p.layout} size="large"/>
      <InputWrapper extra={<div style={{ display: 'flex', flexDirection: 'column' }} className="input-extra">
            {p.denied ? <TikTokDenied /> : <TikTokInfo />}
          </div>} layout={p.layout} className={cx({ [styles.hideLabel]: p.layout === 'vertical' })}>
        <TikTokButtons denied={p.denied}/>
      </InputWrapper>
    </>);
}
function TikTokDenied() {
    return (<InfoBanner id="tiktok-denied" message={$t('TikTok Live Access not granted. Click here to learn more.')} type="info" onClick={() => {
            openConfirmation();
...
function TikTokInfo() {
    return (<>
      ...
      <InfoBanner id="tiktok-info" message={$t("Approvals are solely at TikTok's discretion.")} type="info" style={{ marginTop: '5px', marginBottom: '5px' }}/>
    </>);
}
function TikTokButtons(p) {
    ...
    return (<>
      <Button id="tiktok-locate-key" onClick={openProducer} style={{ marginBottom: '10px', width: '100%' }}>
        {$t('Locate my Stream Key')}
      </Button>

      <Button id="tiktok-application" onClick={() => {
...
//# sourceMappingURL=TiktokEditStreamInfo.jsx.map