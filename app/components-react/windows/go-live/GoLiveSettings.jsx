...
export default function GoLiveSettings() {
    ...
    return (<Row gutter={16} className={styles.settingsRow}>
      
      {shouldShowLeftCol && (<Col span={9} className={styles.leftColumn}>
          ...
          {shouldShowAddDestButton && (<AddDestinationButton type="small" className={styles.columnPadding} onClick={() => Services.SettingsService.actions.showSettings('Stream')}/>)}

          <div className={styles.leftFooter}>
            <PrimaryChatSwitcher className={cx(styles.primaryChat, {
                [styles.disabled]: !shouldShowPrimaryChatSwitcher,
            })} enabledPlatforms={enabledPlatforms} onSetPrimaryChat={setPrimaryChat} primaryChat={primaryChat} suffixIcon={<CaretDownOutlined />} layout="horizontal" logo={false} border={false} disabled={!shouldShowPrimaryChatSwitcher}/>

            <div className={cx(styles.toggleWrapper, { [styles.shiftEnabled]: isStreamShiftMode })}>
              <Tooltip title={$t('Dual Output cannot be used with Stream Shift')} placement="top" lightShadow={true} disabled={isDualOutputSwitchDisabled || !isPrime}>
                <DualOutputToggle className={styles.featureToggle} checkboxClassname={styles.featureCheckbox} style={{ paddingBottom: '10px', width: featureCheckboxWidth }} disabled={isStreamShiftMode} tooltipDisabled={isStreamShiftMode} label={$t('Dual Output')} type="single" lightShadow/>
              </Tooltip>
              <Tooltip title={isPrime
                ...
                <StreamShiftToggle className={styles.featureToggle} checkboxClassname={styles.featureCheckbox} style={{ width: featureCheckboxWidth }} disabled={isStreamShiftDisabled || !isPrime}/>
              </Tooltip>
            </div>
          </div>
        </Col>)}
      ...
      <Col span={shouldShowLeftCol ? 15 : 24} className={cx(styles.rightColumn, !shouldShowLeftCol && styles.destinationMode)}>
        <Spinner visible={isLoading} relative/>
        <GoLiveError />
        {shouldShowSettings && (<>
            <Scrollable style={{ height: '92%' }} snapToWindowEdge>
              {recommendedColorSpaceWarnings && (<ColorSpaceWarnings warnings={recommendedColorSpaceWarnings}/>)}
              
              <PlatformSettings />
              
              {isAdvancedMode && <div className={styles.spacer}/>}
              
              {!!canUseOptimizedProfile && (<Section isSimpleMode={!isAdvancedMode} title={$t('Extras')}>
...
//# sourceMappingURL=GoLiveSettings.jsx.map