...
export function DestinationSwitchers() {
    ...
    return (<div className={cx(styles.switchWrapper, styles.columnPadding)}>
      {platforms.map((platform, ind) => (<DestinationSwitcher key={platform} destination={platform} enabled={isPrime || isDualOutputMode
                ? isEnabled(platform)
                : isEnabled(platform) && (isPrimaryPlatform(platform) || platform === 'tiktok')} onChange={enabled => togglePlatform(platform, enabled)} switchDisabled={!isEnabled(platform) && disableNonUltraSwitchers} isDualOutputMode={isDualOutputMode} index={ind}/>))}

      {customDestinations === null || customDestinations === void 0 ? void 0 : customDestinations.map((dest, ind) => (<DestinationSwitcher key={ind} destination={dest} enabled={dest.enabled && !disableCustomDestinationSwitchers} onChange={enabled => toggleDestination(ind, enabled)} switchDisabled={disableCustomDestinationSwitchers || (!dest.enabled && disableNonUltraSwitchers)} isDualOutputMode={isDualOutputMode} index={ind}/>))}

      {unlinkedPlatforms.map((platform, ind) => (<DestinationSwitcher key={platform} destination={platform} enabled={false} onChange={() => { }} switchDisabled={true} isDualOutputMode={isDualOutputMode} isUnlinked={true} index={ind}/>))}
    </div>);
}
    ...
    const { title, description, Controller, Logo } = (() => {
        ...
        if (platform) {
            ...
            return {
                ...
                Logo: () => (<PlatformLogo platform={platform} className={cx(styles[`platform-logo-${platform}`])}/>),
                Controller: () => (<SwitchInput inputRef={switchInputRef} value={p.enabled} name={platform} disabled={disabled} uncontrolled label={label} nolabel className="platform-switch" color="secondary" size="default" skipWrapperAttrs={true}/>),
            };
        }
        else {
            ...
            return {
                ...
                Controller: () => (<SwitchInput inputRef={switchInputRef} value={p.enabled} name={name} disabled={disabled} uncontrolled label={label} nolabel className="destination-switch" color="secondary" size="default" skipWrapperAttrs={true}/>),
            };
        }
    })();
    return (<div ref={containerRef} className={cx('single-output-card', styles.platformSwitcher, {
      ...
      {p.isDualOutputMode && !(p === null || p === void 0 ? void 0 : p.isUnlinked) && (<div className={styles.displaySelectorWrapper} onClick={e => e.stopPropagation()}>
          <DisplaySelector title={title} nolabel className={styles.dualOutputDisplaySelector} platform={platform} index={p.index}/>
        </div>)}

      
      {(p === null || p === void 0 ? void 0 : p.isUnlinked) && platform && (<ConnectButton platform={platform} className={styles.connectButton}/>)}
    </div>);
});
//# sourceMappingURL=DestinationSwitchers.jsx.map