...
export default function GoLiveError() {
    ...
    function render() {
        ...
        switch (type) {
            ...
            default:
                return React.createElement(MessageLayout, { error: error });
        }
    }
    ...
    function renderPrepopulateError(error, message) {
        ...
        return (React.createElement(MessageLayout, { error: error, message: message !== null && message !== void 0 ? message : $t('Failed to fetch settings from %{platformName}', { platformName }) },
            React.createElement(Translate, { message: $t('prepopulateStreamSettingsError') },
                React.createElement("a", { slot: "fetchAgainLink", className: css.link, onClick: () => actions.prepopulateInfo() }),
                React.createElement("a", { slot: "justGoLiveLink", className: css.link, onClick: () => actions.goLive() }))));
    }
    function renderPrimeRequiredError() {
        return (React.createElement(MessageLayout, { type: 'error', message: $t('Multistreaming to these platforms requires Ultra') },
            React.createElement("button", { className: "button button--prime", onClick: () => MagicLinkService.actions.linkToPrime('slobs-multistream-error') }, $t('Become a Ultra member'))));
    }
    function renderRemergeError(error, relogin = false, message) {
        ...
        return (React.createElement(MessageLayout, { type: "info", message: $t('Failed to update %{platform} account', { platform }) },
            React.createElement("p", null, description),
            React.createElement(Translate, { message: "<unlink>Unlink here</unlink>" },
                React.createElement("a", { slot: "unlink", onClick: () => {
                        var _a;
    ...
    function renderSettingsUpdateError(error) {
        ...
        return (React.createElement(MessageLayout, { error: error, message: $t('Failed to update settings for %{platformName}', { platformName }) },
            React.createElement(Translate, { message: $t('updateStreamSettingsError') },
                React.createElement("a", { slot: "tryAgainLink", className: css.link, onClick: tryAgain }),
                React.createElement("a", { slot: "justGoLiveLink", className: css.link, onClick: skipSettingsUpdateAndGoLive }))));
    }
    function renderRestreamError(error) {
        ...
        const details = !error.details || error.details === ''
            ...
            : error.details.split('\n');
        return (React.createElement(MessageLayout, { error: error, hasButton: true, message: $t('Please try again. If the issue persists, you can stream directly to a single platform instead or click the button below to bypass and go live.') },
            `${$t('Issues')}:`,
            React.createElement("ul", null, details.map((detail, index) => (React.createElement("li", { key: `detail-${index}` }, detail)))),
            React.createElement("button", { className: "button button--warn", style: { marginTop: '8px' }, onClick: skipSettingsUpdateAndGoLive }, $t('Bypass and Go Live'))));
    }
    function renderDualOutputError(error) {
        return (React.createElement(MessageLayout, { error: error }, $t('Please try again. If the issue persists, you can stream in single output mode instead.')));
    }
    function renderYoutubeStreamingDisabled(error) {
        return (React.createElement(MessageLayout, { message: error.message },
            $t('Please enable your account for live streaming, and wait 24 hours before attempting to stream.'),
            React.createElement("br", null),
            React.createElement("button", { className: "button button--warn", style: { marginTop: '8px' }, onClick: () => YoutubeService.actions.openYoutubeEnable() }, $t('Enable Live Streaming'))));
    }
    function renderYoutubeTokenExpiredError(error) {
        return (React.createElement(MessageLayout, { error: error, hasButton: true },
            React.createElement(Translate, { message: $t('YouTube token has expired, re-login or re-merge YouTube account') },
                React.createElement("button", { slot: "connectButton", className: "button button--warn", onClick: () => navigatePlatformMerge('youtube') }))));
    }
    function renderTikTokOAuthExpiredError(error) {
        assertIsDefined(error.platform);
        return (React.createElement(MessageLayout, { error: error, hasButton: true },
            React.createElement(Translate, { message: $t('Failed to authenticate with TikTok, re-login or re-merge TikTok account') },
                React.createElement("button", { slot: "connectButton", className: "button button--warn", onClick: () => navigatePlatformMerge('tiktok') }))));
    }
    function renderTikTokCredentialsFailedError(error) {
        return (React.createElement(MessageLayout, { error: error, message: $t('Failed to generate TikTok stream credentials. Confirm Live Access with TikTok.') }));
    }
    function renderTikTokScopeMissingError(error) {
        return (React.createElement(MessageLayout, { error: error, message: $t('Your TikTok account is not enabled for live streaming.') }));
    }
    function renderTikTokScopeOutdatedError(error) {
        return renderRemergeError(error);
    }
    function renderMachineLockedError(error) {
        return (React.createElement(MessageLayout, { error: error }, $t('You could try locking and unlocking your computer to fix this error.')));
    }
    function renderFacebookNotEligibleForStreamingError() {
        return (React.createElement(MessageLayout, null, $t("You're not eligible to Go Live, your profile needs to be at least 60 days old and your page needs to have at least 100 followers. Click the notification to learn more.")));
    }
    function renderKickStreamKeyMissingError(error) {
        ...
        return (React.createElement(MessageLayout, { error: error, type: "info", message: error.message },
            React.createElement("div", null,
                ...
                React.createElement("ol", null,
                    React.createElement("li", null, $t('Login to Kick')),
                    React.createElement("li", null,
                        React.createElement(Translate, { message: $t('Go to: <link></link>'), renderSlots: {
                                link: () => (React.createElement("a", { style: { textDecoration: 'underline' }, onClick: openKickSettings }, 'https://kick.com/settings/stream')),
                            } })),
                    React.createElement("li", null, $t('Click on the \"eye\" icon to see your stream key')),
                    React.createElement("li", null, $t('Put in 2FA'))),
...
//# sourceMappingURL=GoLiveError.js.map