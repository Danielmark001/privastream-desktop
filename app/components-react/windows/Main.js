...
export default function Main() {
    ...
    if (!uiReady)
        return React.createElement("div", { className: cx(styles.main, theme) });
    const Component = appPages[page];
    return (React.createElement("div", { className: cx(styles.main, theme, 'react'), id: "mainWrapper", ref: mainWindowEl, onDrop: (ev) => onDropHandler(ev) },
        React.createElement(TitleBar, { windowId: "main", className: cx({ [styles.titlebarError]: errorAlert }) }),
        React.createElement("div", { className: cx(styles.mainContents, {
            ...
            page !== 'Onboarding' && !showLoadingSpinner && (React.createElement("div", { className: styles.sideNavContainer },
                React.createElement(SideNav, { isVisionRunning: isVisionRunning }))),
            renderDock && leftDock && (React.createElement(LiveDockContainer, { max: maxDockWidth, min: minDockWidth, width: dockWidth, setCollapsed: setCollapsed, setLiveDockWidth: setDockWidth, onLeft: true })),
            React.createElement("div", { className: cx(styles.mainMiddle, { [styles.mainMiddleCompact]: compactView }), ref: mainMiddleEl },
                !showLoadingSpinner && (React.createElement("div", { className: styles.mainPageContainer },
                    React.createElement(Component, { params: params, onTotalWidth: (width) => handleEditorWidth(width) }))),
                !applicationLoading && page !== 'Onboarding' && (React.createElement("div", { style: { display: 'flex', minWidth: '0px', gridRow: '2 / span 1' } },
                    React.createElement(StudioFooter, null)))),
            renderDock && !leftDock && (React.createElement(LiveDockContainer, { max: maxDockWidth, min: minDockWidth, width: dockWidth, setCollapsed: setCollapsed, setLiveDockWidth: setDockWidth }))),
        React.createElement(ModalWrapper, { renderFn: modalOptions.current.renderFn }),
        React.createElement(Animation, { transitionName: "ant-fade" }, (!uiReady || showLoadingSpinner) && (React.createElement("div", { className: cx(styles.mainLoading, { [styles.initialLoading]: !uiReady }) },
            React.createElement(Loader, null))))));
}
function LiveDockContainer(p) {
    const isDockCollapsed = useRealmObject(Services.CustomizationService.state).livedockCollapsed;
    function Chevron() {
        return (React.createElement("div", { className: cx(styles.liveDockChevron, p.onLeft && styles.left), onClick: () => p.setCollapsed(!isDockCollapsed) },
    ...
    return (React.createElement(Animation, { transitionName: transitionName, transitionAppear: true },
        isDockCollapsed && (React.createElement("div", { className: cx(styles.liveDockCollapsed, p.onLeft && styles.left), key: "collapsed" },
            React.createElement(Chevron, null))),
        !isDockCollapsed && (React.createElement(ResizeBar, { position: p.onLeft ? 'left' : 'right', onInput: (val) => p.setLiveDockWidth(val), max: p.max, min: p.min, value: p.width, transformScale: 1, key: "expanded" },
            React.createElement("div", { className: cx(styles.liveDockContainer, p.onLeft && styles.left), style: { width: `${p.width}px` } },
