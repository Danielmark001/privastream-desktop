...
export default function Projector() {
    ...
    return (<div className={styles.projectorContainer}>
      {fullscreen && (<div className={styles.projectorFullscreen}>
          <Display sourceId={sourceId} renderingMode={renderingMode} style={{ flexGrow: 1 }}/>
        </div>)}
      {!fullscreen && (<ModalLayout bodyStyle={{ padding: 0 }} hideFooter>
          <div className={styles.projectorWindowed}>
            <Scrollable className={styles.buttonContainer} style={{ height: 40 }}>
              <div className={styles.projectorButtons}>
                {remote.screen.getAllDisplays().map((display, idx) => (<button className="button button--trans" key={display.id} onClick={() => enterFullscreen(display)}>
