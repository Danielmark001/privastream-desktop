...
export default function SourceGrid(p) {
    ...
    const toSourceEl = (source) => (React.createElement(SourceTag, { key: source.value, type: source.value, essential: true, excludeWrap: excludeWrap }));
    const toWidgetEl = (widget, { essential = false, hideShortDescription = false } = {}) => widget === 'streamlabel' ? (React.createElement(SourceTag, { key: "streamlabel", name: $t('Stream Label'), type: "streamlabel", essential: true, hideShortDescription: p.activeTab !== 'all', excludeWrap: excludeWrap })) : (React.createElement(SourceTag, { key: widget, type: widget, excludeWrap: excludeWrap, essential: essential, hideShortDescription: hideShortDescription }));
    const essentialSourcesList = useMemo(() => {
        if (essentialSources.essentialDefaults.length || essentialSources.essentialWidgets.length) {
            return (React.createElement(React.Fragment, null,
                essentialSources.essentialDefaults.map(source => (React.createElement(SourceTag, { key: source.value, type: source.value, essential: true, excludeWrap: excludeWrap }))),
                isLoggedIn &&
    ...
    const widgetList = useMemo(() => {
        ...
        return (React.createElement(React.Fragment, null, !isLoggedIn ? (React.createElement(Empty, { description: $t('You must be logged in to use Widgets'), image: $i(`images/sleeping-kevin-${demoMode}.png`) },
            React.createElement(Button, { onClick: handleAuth }, $t('Click here to log in')))) : (list)));
    ...
    const widgetGroupedList = useMemo(() => {
        return (React.createElement(React.Fragment, null, !isLoggedIn ? (React.createElement(Empty, { description: $t('You must be logged in to use Widgets'), image: $i(`images/sleeping-kevin-${demoMode}.png`) },
            React.createElement(Button, { onClick: handleAuth }, $t('Click here to log in')))) : (React.createElement(Collapse, { ghost: true, activeKey: widgetSections, onChange: xs => setWidgetExpandedSections(xs) },
            nonEmptyPanel({
                id: 'essentialWidgets',
    ...
    const appsList = useMemo(() => (React.createElement(React.Fragment, null, availableAppSources.map(app => (React.createElement(SourceTag, { key: `${app.appId}${app.source.id}`, name: app.source.name, type: "app_source", appId: app.appId, appSourceId: app.source.id, excludeWrap: excludeWrap }))))), [availableAppSources, excludeWrap]);
    const groupedSources = useMemo(() => (React.createElement(React.Fragment, null,
        nonEmptyPanel({
            id: 'captureSources',
        ...
        if (showContent('general')) {
            return (React.createElement(React.Fragment, null,
                React.createElement(Col, { span: 24 },
                    React.createElement(Collapse, { ghost: true, activeKey: expandedSections, onChange: xs => setExpandedSections(xs) }, groupedSources))));
        }
        else if (showContent('widgets')) {
            return (React.createElement(React.Fragment, null,
                React.createElement(Col, { span: 24 }, widgetGroupedList)));
        }
        else if (showContent('apps')) {
            return (React.createElement(React.Fragment, null,
                React.createElement(Col, { span: 24 },
                    React.createElement(PageHeader, { style: { paddingLeft: 0 }, title: $t('Apps') })),
                appsList));
        }
    }, [p.activeTab, availableAppSources, appsList, widgetList]);
    return (React.createElement(Scrollable, { style: { height: 'calc(100% - 64px)' }, className: styles.sourceGrid },
        React.createElement(Row, { gutter: [8, 8], style: { marginLeft: '8px', marginRight: '8px', paddingBottom: '24px' } }, p.activeTab === 'all' ? (React.createElement(React.Fragment, null,
            React.createElement(Col, { span: 24 },
                React.createElement(Collapse, { ghost: true, activeKey: expandedSections, onChange: xs => setExpandedSections(xs) },
                    nonEmptyPanel({
                        id: 'essentialSources',
...
const nonEmptyPanel = ({ src, id, header, testId, }) => {
    ...
    return (React.createElement(Panel, { header: header, key: id },
        React.createElement("div", { className: "collapse-section", "data-testid": testId }, src)));
};
//# sourceMappingURL=SourceGrid.js.map