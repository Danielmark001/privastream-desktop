...
export default function Settings() {
    ...
    return (<ModalLayout bodyClassName={styles.settings}>
      <Scrollable className={styles.settingsNav}>
        <div style={{ padding: '0 24px 12px 24px' }}>
          <TextInput prefix={<i className="icon-search"/>} placeholder={$t('Search')} value={searchStr} onChange={onSearchInput} uncontrolled={false} nowrap/>
        </div>
        <Menu mode="inline" selectedKeys={[currentTab]} onClick={handleMenuNavigation} style={{ paddingBottom: '46px' }}>
          {categories.map(cat => {
            const config = SETTINGS_CONFIG[cat];
            return (<Menu.Item key={cat} icon={<i className={config.icon}/>}>
                <div style={{ display: 'flex' }} onClick={() => dismiss(cat)} data-name="settings-nav-item">
                  {$t(cat)}
                  {config.dismissable && showDismissable(config.dismissable) && (<DismissableBadge dismissableKey={config.dismissable}/>)}
                </div>
              </Menu.Item>);
        })}
        </Menu>
      ...
      <Scrollable className={styles.settingsContainer} snapToWindowEdge>
        <div className={styles.settingsContent}>
          <SearchablePages onSearchCompleted={handleSearchCompleted} pages={categories} page={currentTab} searchStr={searchStr}>
