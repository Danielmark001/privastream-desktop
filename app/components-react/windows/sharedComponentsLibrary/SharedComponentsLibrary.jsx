...
function Examples() {
    ...
    return (<Form layout={layout}>
      <Example title="TextInput">
        ...
        <TextInput label="Controlled" uncontrolled={false} placeholder={placeholder} {...bind.textVal}/>
        ...
        <TextInput label="With addons" addonBefore="http://" addonAfter=".com" {...globalProps} {...bind.textVal}/>
      </Example>

      <Example title="Number Input">
        <NumberInput label="Basic" {...globalProps} {...bind.numberVal}/>
        <NumberInput label="Min = 0, Max = 10" min={0} max={10} {...globalProps} {...bind.numberVal}/>
      </Example>

      <Example title="Textarea Input">
        <TextAreaInput label="Basic" {...globalProps} {...bind.textAreaVal}/>
        <TextAreaInput label="Show Count" {...globalProps} {...bind.textAreaVal} showCount maxLength={50}/>
      ...
      <Example title="List Input">
        ...
        <ListInput label="With search" {...globalProps} {...bind.listVal} options={s.listOptions} showSearch/>
        <ListInput label="Allow Clear" {...globalProps} {...bind.listVal} options={s.listOptions} allowClear/>
        <ListInput label="Custom Empty" {...globalProps} {...bind.listVal2} options={s.listOptions2}/>
      </Example>

      <Example title="Tags Input">
        <TagsInput label="Basic" {...globalProps} {...bind.tagsVal} options={s.tagsOptions}/>
        <TagsInput label="Custom Tag Render" {...globalProps} {...bind.tagsVal} options={s.tagsOptions} tagRender={(tagProps, tag) => (<Tag {...tagProps} color={tag.label.toLowerCase()}>
              {tag.label}
            </Tag>)}/>
        <TagsInput label="Custom Option Render" {...globalProps} {...bind.tagsVal} options={s.tagsOptions} optionRender={opt => (<Row gutter={16} style={{ color: opt.label.toLowerCase() }}>
              <Col>{opt.value}</Col>
              <Col>{opt.label}</Col>
            </Row>)}/>
      </Example>
      ...
      <Example title="SwitchInput">
        ...
        <SwitchInput label="With text" {...globalProps} {...bind.switcherVal} checkedChildren="Enabled" unCheckedChildren="Disabled"/>
      </Example>
      ...
      <Example title="Slider Input">
        ...
        <SliderInput label="Without Number Input" min={0} max={10} hasNumberInput={false} {...bind.sliderVal}/>
        <SliderInput label="Debounced" min={0} max={10} debounce={300} {...globalProps} {...bind.sliderVal}/>
      </Example>
      ...
      <Example title="Modals">
        <Space>
          ...
          <Button onClick={() => confirmAsync('This is Alert').then(confirmed => alertAsync(confirmed ? 'Confirmed' : 'Not confirmed'))}>
            Show Confirm
          </Button>
        </Space>
      </Example>
...
export function Example(p) {
    ...
    return (<Container background={background} title={p.title}>
      {background !== 'error' && (<InputWrapper>
          <h2>{p.title}</h2>
        </InputWrapper>)}
...
function Container(p) {
    return (<div>
      ...
      {p.background === 'error' && (<Alert type="error" message={p.title} description={p.children} style={{ marginBottom: '24px' }}/>)}
    </div>);
}
function SettingsPanel() {
    ...
    return (<Form layout="vertical" style={{
            position: 'fixed',
            height: '100%',
      ...
      <ListInput label="Theme" options={createOptions(['night-theme', 'day-theme', 'prime-dark', 'prime-light'])} {...bind.theme}/>
      <ListInput label="Layout" options={createOptions(['horizontal', 'vertical', 'inline'])} {...bind.layout}/>
      <ListInput label="Background" options={createOptions(['none', 'section', 'section-alt', 'error'])} {...bind.background}/>
      <ListInput label="Size" options={createOptions(['default', 'large', 'small'])} {...bind.size}/>
...
//# sourceMappingURL=SharedComponentsLibrary.jsx.map