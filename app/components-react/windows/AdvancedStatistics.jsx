...
export default function AdvancedStatistics() {
    ...
    return (<ModalLayout hideFooter>
      <div slot="content" className={styles.container} data-syncstatus={syncStatus}>
        ...
        <NotificationsArea notifications={notifications} onNotificationClickHandler={onNotificationClickHandler}/>
      </div>
    </ModalLayout>);
}
function StatusBar(p) {
    return (<div>
      <h2>Status</h2>
      <div className={cx(styles.status, p.status.type === 'error' ? styles.error : '', p.status.type === 'warning' ? styles.warning : '', p.status.type === 'success' ? styles.success : '')}>
...
function NotificationsArea(p) {
    return (<div className={styles.section} style={{ height: '100%' }}>
      <h2>{$t('Performance Notifications')}</h2>
      <Scrollable className={styles.notificationContainer} style={{ height: '100%', width: '100%' }} isResizable={false}>
        {p.notifications.map(notification => (<div className={cx(styles.notification, styles.hasAction)} data-name="notification" key={notification.id} onClick={() => {
        ...
        {p.notifications.length === 0 && (<div className={styles.notificationEmpty}>
            <div className="message">{$t("You don't have any notifications")}</div>
          </div>)}
      </Scrollable>
      <p className={styles.description}>
        {$t('When Streamlabs detects performance issues with your stream, such as dropped frames, lagged frames, or a stream disconnection, troubleshooting notifications will be sent to you here.')}
      </p>
    </div>);
}
function IconForNotification(p) {
    return (<div className="icon">
      {p.notification.subType === ENotificationSubType.DISCONNECTED && (<i className={cx('icon-disconnected', styles.errorText)}/>)}
      {p.notification.subType === ENotificationSubType.SKIPPED && (<i className={cx('icon-reset icon-skipped-frame', styles.warningText)}/>)}
      {p.notification.subType === ENotificationSubType.LAGGED && (<i className={cx('icon-time', styles.warningText)}/>)}
      {p.notification.subType === ENotificationSubType.DROPPED && (<i className={cx('icon-back-alt icon-down-arrow', styles.warningText)}/>)}
    </div>);
}
//# sourceMappingURL=AdvancedStatistics.jsx.map