...
export default function AddSource() {
    ...
    function Footer() {
        return (React.createElement(React.Fragment, null,
            React.createElement("div", { className: styles.newSourceToggle }, canCreateNew && (React.createElement(SwitchInput, { value: overrideExistingSource, onChange: setOverrideExistingSource, label: $t('Add a new source instead') }))),
            React.createElement("button", { className: "button button--default", onClick: close, style: { marginRight: '6px' } }, $t('Cancel')),
            React.createElement("button", { className: "button button--action", onClick: handleSubmit }, $t('Add Source'))));
    }
    return (React.createElement(ModalLayout, { footer: React.createElement(Footer, null) },
        React.createElement("div", { className: styles.container },
            !isNewSource() && (React.createElement(React.Fragment, null,
                React.createElement("div", null,
                    React.createElement("h4", null,
                        $t('Add Existing Source'),
                        sourceAddOptions.propertiesManager === 'widget' && existingSources.length > 0 && (React.createElement("span", { className: styles.recommendedLabel }, $t('Recommended')))),
                    React.createElement(Scrollable, { className: styles.menuContainer },
                        React.createElement(Menu, { mode: "vertical", selectedKeys: [selectedSourceId], onClick: ({ key }) => {
                                setSelectedSourceId(key);
                            }, className: styles.menu }, existingSources.map(source => (React.createElement(Menu.Item, { key: source.value }, source.name)))))),
                selectedSourceId && (React.createElement(Display, { sourceId: selectedSourceId, style: { width: '200px', height: '200px' } })))),
            isNewSource() && (React.createElement(Form, { form: form, name: "addNewSourceForm", onFinish: addNew },
                React.createElement("h4", null, $t('Add New Source')),
                React.createElement(TextInput, { label: $t('Please enter the name of the source'), value: name, onInput: setName, name: "newSourceName", autoFocus: true, required: true, uncontrolled: false, layout: "vertical" }))))));
}
