...
export default function MultistreamChatInfo() {
    ...
    return (<ModalLayout className={styles.chatInfoContainer}>
      ...
      {platforms.map(platform => (<Row key={`${platform.name.toLowerCase().split(' ').join('-')}-chat-info`} className={styles.chatInfoRow}>
          <Col span={10} className={cx(styles.chatInfoCol, styles.platform)}>
            <PlatformLogo platform={platform.icon} className={cx(styles.chatPlatformIcon, styles[`platform-logo-${platform.icon}`])}/>
            {platform.name}
          </Col>
          <Col span={7} className={cx(styles.chatInfoCol, {
...
//# sourceMappingURL=MultistreamChatInfo.jsx.map