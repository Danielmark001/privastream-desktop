...
export default function Settings() {
    const { SettingsService, NavigationService, UserService, DualOutputService, WindowsService, DismissablesService, UsageStatisticsService, } = Services;
    ...
    const { isPrime, isLoggedIn, username, platform, showDismissable } = useVuex(() => {
        ...
        return ({
            ...
            showDismissable: (value) => DismissablesService.views.shouldShow(value),
        });
    });
    ...
    return (React.createElement(ModalLayout, { bodyClassName: styles.settings },
        React.createElement(Scrollable, { className: styles.settingsNav },
            React.createElement("div", { style: { padding: '0 24px 12px 24px' } },
                React.createElement(TextInput, { prefix: React.createElement("i", { className: "icon-search" }), placeholder: $t('Search'), value: searchStr, onChange: onSearchInput, uncontrolled: false, nowrap: true })),
            React.createElement(Menu, { mode: "inline", selectedKeys: [currentTab], onClick: handleMenuNavigation, style: { paddingBottom: '46px' } }, categories.map(cat => {
                const config = SETTINGS_CONFIG[cat];
                return (React.createElement(Menu.Item, { key: cat, icon: React.createElement("i", { className: config.icon }) },
                    React.createElement("div", { style: { display: 'flex' }, onClick: () => dismiss(cat), "data-name": "settings-nav-item" },
                        $t(cat),
                        config.dismissable && showDismissable(config.dismissable) && (React.createElement(DismissableBadge, { dismissableKey: config.dismissable })))));
            })),
            React.createElement("div", { className: styles.settingsAuth, onClick: handleAuth },
                React.createElement("i", { className: isLoggedIn ? 'fas fa-sign-out-alt' : 'fas fa-sign-in-alt' }),
                React.createElement("strong", null, isLoggedIn ? $t('Log Out') : $t('Log In')),
                isLoggedIn && platform && React.createElement(PlatformLogo, { platform: platform, size: "small" }),
                isLoggedIn && React.createElement("span", null, username))),
        React.createElement(Scrollable, { className: styles.settingsContainer, snapToWindowEdge: true },
            React.createElement("div", { className: styles.settingsContent },
                React.createElement(SearchablePages, { onSearchCompleted: handleSearchCompleted, pages: categories, page: currentTab, searchStr: searchStr },
                    React.createElement(SettingsContent, { globalSearchStr: searchStr }))))));
}
