...
export default function GuestCamProperties() {
    ...
    return (<GuestCamCtx.Provider value={controller}>
...
function GuestCamPropertiesModal() {
    ...
    if (!loggedIn) {
        return (<ModalLayout>
        <Alert type="error" showIcon={true} closable={false} message={<div>{$t('You must be logged in to use Collab Cam.')}</div>}/>
      </ModalLayout>);
    }
    return (<ModalLayout scrollable>
      <Tabs destroyInactiveTabPane={true} defaultActiveKey={defaultTab}>
        <Tabs.TabPane tab={$t('Settings')} key="settings">
          <Form layout="inline">
            {joinAsGuest ? (<div style={{ height: 32, margin: '10px 0 10px' }}>
                <div>
                  <b>{$t('Connected To Host:')}</b>{' '}
                  <span style={{ color: 'var(--title)' }}>{hostName}</span>
                  <Tooltip title={$t("You are connected as a guest using someone else's invite link. To leave, click the Disconnect button.")}>
                    <QuestionCircleOutlined style={{ marginLeft: 6 }}/>
                  </Tooltip>
                  <button style={{ marginLeft: 10 }} className="button button--soft-warning" onClick={disconnectFromHost}>
                    {$t('Disconnect')}
                  </button>
                </div>
              </div>) : (<div style={{ display: 'flex', width: '100%', margin: '10px 0' }}>
                <TextInput readOnly value={inviteUrl} label={$t('Invite URL')} style={{ flexGrow: 1 }} addonAfter={<Tooltip trigger="click" title={$t('Copied!')}>
                ...
                <Button disabled={regeneratingLink} onClick={regenerateLink} style={{ width: 180 }}>
                  ...
                  {regeneratingLink && (<i className="fa fa-spinner fa-pulse" style={{ marginLeft: 8 }}/>)}
                </Button>
              </div>)}
            {!joinAsGuest && (<div style={{ margin: '10px 0 0', width: '100%' }}>
                <span>{$t('Guests')}</span>
                <span style={{
                marginLeft: 8,
                ...
                {shouldShowPrimeUpgrade && (<span style={{ marginLeft: 8, cursor: 'pointer' }} onClick={() => MagicLinkService.actions.linkToPrime('desktop-collab-cam')}>
                    <UltraIcon style={{
            ...
            <h2 style={{ marginTop: 20 }}>
              {$t('The webcam and microphone source you select below will be broadcast to your guests.')}
            </h2>
            <div style={{
            display: 'flex',
            width: '100%',
            ...
            justifyContent: 'space-between',
              ...
              <ListInput label={$t('Webcam Source')} options={videoProducerSourceOptions} value={videoProducerSourceId} onChange={s => GuestCamService.actions.setVideoSource(s)} style={{ width: '48%', margin: 0 }}/>
              <ListInput label={$t('Microphone Source')} options={audioProducerSourceOptions} value={audioProducerSourceId} onChange={s => GuestCamService.actions.setAudioSource(s)} style={{ width: '48%', margin: 0 }}/>
            </div>
            <div style={{
            display: 'flex',
            width: '100%',
            margin: '10px 0',
              ...
              <ListInput label={$t('Share Video Source (Optional)')} options={screenshareProducerSourceOptions} value={screenshareProducerSourceId} onChange={s => GuestCamService.actions.setScreenshareSource(s)} style={{ width: '48%', margin: 0 }}/>
              {screenshareProducerSourceId && (<button className="button button--soft-warning" onClick={() => GuestCamService.actions.setScreenshareSource('')} style={{ marginLeft: 30 }}>
                  {$t('Stop Sharing')}
                </button>)}
            </div>
          </Form>
          {(!videoProducerSource || !audioProducerSource) && (<Alert type="error" showIcon={true} closable={false} message={<div style={{ color: 'var(--paragraph)' }}>
                  {!videoProducerSource && (<div>
                      {$t('No webcam source is selected. Your guest will not be able to see you.')}
                    </div>)}
                  {!audioProducerSource && (<div>
                      {$t('No microphone source is selected. Your guest will not be able to hear you.')}
                    </div>)}
                </div>}/>)}
        </Tabs.TabPane>
        {guests.map(guest => {
            return (<Tabs.TabPane tab={getGuestLabel(guest)} key={guest.remoteProducer.streamId}>
              <GuestPane guest={guest}/>
            </Tabs.TabPane>);
        })}
      </Tabs>
      <Modal visible={!produceOk} getContainer={false} closable={false} okText={getModalButtonText()} onOk={() => {
            if (sourceExists) {
...
function GuestSourceSelector(p) {
    ...
    return (<ListInput options={availableSources} value={sourceId} label={$t('Assign to Source')} listHeight={120} onChange={setSourceId} style={p.style}/>);
}
function DisconnectModal(p) {
    const [regen, setRegen] = useState(true);
    return (<>
      <p>
        {$t('If you want to prevent this guest from rejoining, you should also regenerate your invite link.')}
      </p>
      <Form>
        <CheckboxInput label={$t('Regenerate Link')} value={regen} onChange={(val) => {
            setRegen(val);
        ...
        <Form layout="inline">
          <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
            ...
            <div style={{ flexGrow: 1, margin: '0 20px 20px' }}>
              <SliderInput label={$t('Volume')} value={volume} onChange={setVolume} min={0} max={1} debounce={500} step={0.01} tipFormatter={v => `${(v * 100).toFixed(0)}%`} tooltipPlacement="bottom" style={{ width: '100%' }}/>
            </div>
          </div>
          <div style={{ width: '100%', marginLeft: 20 }}>
            ...
            margin: '20px 0',
              ...
              <Button onClick={setVisible} style={{ width: 160, marginRight: 40 }} type={!visible ? 'primary' : 'default'}>
                {visible ? $t('Hide on Stream') : $t('Show on Stream')}
              </Button>
              <button className="button button--soft-warning" style={{ width: 160 }} onClick={onDisconnectClick}>
                {$t('Disconnect')}
              </button>
            </div>
          </div>
...
function GuestDisplay(p) {
    ...
    return (<div style={{ background: 'var(--section)', borderRadius: '8px 8px 0 0', height: 280 }}>
...
function FirstTimeModalContent() {
    return (<>
      ...
      <h3>{$t('Step 1')}</h3>
      <p>
        {$t("Copy and share a link with your guest. When they join, they'll be able to see and hear you.")}
      </p>
      <h3>{$t('Step 2')}</h3>
      <p>
        {$t("Verify their identity in the preview area. When ready, add them to your stream by clicking 'Show on Stream'.")}
      </p>
      <h3>{$t('Step 3')}</h3>
      <p>
        {$t('Enjoy your stream! Adjust their volume, create new links, and change your mic and camera from the properties window.')}
      </p>
      <Alert message={<div style={{ color: 'var(--info)' }}>
            <ExclamationCircleOutlined style={{ color: 'var(--info)', marginRight: 8 }}/>
            {$t("Don't share your invite link with anyone you don't want on your stream. You can invalidate an old link by generating a new one. Do not show this window on stream.")}
          </div>} type="info" closable={false} showIcon={false} banner/>
      <a style={{ display: 'inline-block', marginTop: 10 }} onClick={() => remote.shell.openExternal('https://streamlabs.com/collab-cam')}>
        {$t('Learn More')}
      </a>
    </>);
}
...
function MissingSourceModalContent() {
    return (<>
      ...
      <p>
        {$t('At least one Collab Cam source is required to use Collab Cam. Would you like to add one now?')}
      </p>
    </>);
}
//# sourceMappingURL=GuestCamProperties.jsx.map