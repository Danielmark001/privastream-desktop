...
export default function ManageSceneCollections() {
    ...
    return (<ModalLayout onCancel={close}>
      <Layout style={{ height: '100%' }}>
        <Sider width={300}>
          <div>{$t('Your Scene Collections:')}</div>
          <div style={{ width: '96%', marginTop: '8px', marginBottom: '8px' }}>
            <TextInput placeholder={$t('Search Scene Collections')} onChange={setQuery} uncontrolled={false} prefix={<i className="icon-search"/>} nowrap/>
          </div>
          <Scrollable style={{ height: 'calc(100% - 48px)' }}>
            {filteredCollections().map((collection, i) => (<CollectionNode collection={collection} recentlyUpdated={i < 2} key={collection.id}/>))}
          </Scrollable>
        </Sider>
        <Content style={{ paddingLeft: '16px' }}>
          ...
          <div className={styles.buttonContainer}>
            ...
            <button disabled={!isLoggedIn} onClick={goToThemes} className={cx('button', styles.button)}>
            ...
            {!isPrime && (<div onClick={upgradeToPrime} className={cx('button', styles.button, styles.lg)}>
            ...
            <button onClick={importFromObs} className={cx('button', styles.button, { [styles.lg]: isPrime })}>
              <i className="icon-cloud-backup"/>
              <strong>{$t('Import from OBS')}</strong>
              <p>{$t('Load existing scenes from OBS')}</p>
            </button>
            <button onClick={importFromTwitch} className={cx('button', styles.button, { [styles.lg]: isPrime })}>
              <i className="icon-cloud-backup"/>
...
function CollectionNode(p) {
    ...
    return (<div onDoubleClick={makeActive} className={cx(styles.collectionNode, { [styles.active]: isActive })}>
      <span>
        <i className={cx('fab', p.collection.operatingSystem === OS.Windows ? 'fa-windows' : 'fa-apple')}/>
        {p.collection.name}
      </span>
      ...
      <div className={styles.editIcons}>
        ...
        {!duplicating && (<Tooltip title={$t('Duplicate')}>
...
//# sourceMappingURL=ManageSceneCollections.jsx.map