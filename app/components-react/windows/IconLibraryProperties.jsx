...
export default () => {
    ...
    return (<ModalLayout fixedChild={<PreviewImage />} onOk={() => WindowsService.closeChildWindow()}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        {CustomizationService.state.designerMode && (<FileInput onChange={selectFolder} value={folderPath} directory={true} filters={filters}/>)}
        <Scrollable snapToWindowEdge isResizable={false} style={{ height: '100%' }}>
          <div className={styles.cellContainer}>
            {errorState ? (<div>{$t('An error has occured, please try re-opening this window')}</div>) : (folderImages.map(image => (<ImageCell path={image} isSelected={image === selectedIcon} handleClick={selectIcon} key={image}/>)))}
          </div>
        </Scrollable>
      </div>
    </ModalLayout>);
};
const ImageCell = (p) => (<div className={cx(styles.imageCell, {
        [styles.selected]: p.isSelected,
        [styles.large]: p.large,
    })} onClick={() => p.handleClick(p.path)}>
    <img src={p.path}/>
  </div>);
//# sourceMappingURL=IconLibraryProperties.jsx.map