...
export default function MediaGallery() {
    ...
    return (<ModalLayout onOk={handleSelect}>
      <Layout className={styles.container} onDragEnter={onDragEnter} onDragOver={onDragEnter}>
        <Layout.Sider>
          ...
          <Menu mode="inline" onClick={handleCategory} defaultSelectedKeys={['uploads']}>
            {['uploads', 'stock'].map((cat) => (<Menu.Item key={cat}>
                {cat === 'stock' ? $t('Stock Files') : $t('My Uploads')}
              </Menu.Item>))}
          </Menu>
        </Layout.Sider>
        <Layout.Content>
          <PageHeader title={title} subTitle={<Progress percent={Math.round(usagePct * 100)} style={{ width: '200px', display: 'flex', alignItems: 'center' }} trailColor={'var(--section-alt)'} format={displaySpaceRemaining}/>} extra={[
            <i className={cx(styles.toolbarIcon, 'icon-cloud-backup')} onClick={openFilePicker} key="backup"/>,
            <i className={cx(styles.toolbarIcon, 'icon-trash', {
          ...
          <Scrollable style={{ height: 'calc(100% - 18px)' }}>
            {filteredGallery &&
            filteredGallery.map(file => (<Card key={file.href} hoverable onClick={e => selectFile(file, false, e)} onDoubleClick={e => selectFile(file, true, e)} cover={<CardContent file={file}/>} style={{
                    ...
                    file.prime ? (<UltraIcon key="prime" style={{
                  ...
                  <Card.Meta title={file.filename} description={file.size && formatBytes(file.size)}/>
                </Card>))}
          </Scrollable>

          {!filteredGallery && (<div className={styles.emptyBox}>
      ...
      {dragOver && (<div onDragOver={onDragEnter} onDragLeave={onDragLeave} onDrop={handleFileDrop} className={cx(styles.dragOverlay, 'radius')}/>)}
...
function CardContent(p) {
    ...
    if (type === 'image' && (/\.webm$/.test(href) || /\.mp4$/.test(href))) {
        FilePreview = () => (<video autoPlay muted loop src={href} style={{ maxHeight: '148px', maxWidth: '148px' }}/>);
    }
    else if (type === 'image') {
        FilePreview = () => <img src={href} style={{ maxHeight: '148px', maxWidth: '148px' }}/>;
    }
    else if (type === 'audio') {
        FilePreview = () => (<i className="icon-music" style={{
                height: '132px',
                lineHeight: '132px',
                fontSize: '28px',
                textAlign: 'center',
...
//# sourceMappingURL=MediaGallery.jsx.map