...
export default function Notifications() {
    ...
    return (<Scrollable style={{ height: 'calc(93vh - 100px)' }}>
      {!notificationsCount && <h4>{$t("You don't have any notifications")}</h4>}

      {Object.keys(notificationGroups).map((groupName) => (<div key={groupName}>
          {notificationGroups[groupName].length > 0 && (<h4>{groupName === 'unread' ? $t('New Notifications') : $t('Log')}</h4>)}
          {notificationGroups[groupName].map((notify) => (<div key={notify.id} onClick={() => onNotificationClickHandler(notify.id)} className={cx(styles.notification, {
