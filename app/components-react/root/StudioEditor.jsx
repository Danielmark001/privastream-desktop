...
export default function StudioEditor() {
    ...
    return (<div className={styles.mainContainer} ref={studioModeRef}>
      {displayEnabled && (<div className={cx(styles.studioModeContainer, { [styles.stacked]: studioModeStacked })}>
          {v.studioMode && <StudioModeControls stacked={studioModeStacked}/>}
          {v.dualOutputMode && (<DualOutputControls stacked={studioModeStacked} isRecording={v.isRecording}/>)}
          <div className={cx(styles.studioDisplayContainer, { [styles.stacked]: studioModeStacked })}>
            {v.showHorizontalDisplay && (<div className={cx(styles.studioEditorDisplayContainer, 'noselect')} style={{ cursor: v.cursor }} onMouseDown={(event) => eventHandlers.onMouseDown(event, 'horizontal')} onMouseUp={(event) => eventHandlers.onMouseUp(event, 'horizontal')} onMouseEnter={(event) => eventHandlers.onMouseEnter(event, 'horizontal')} onMouseMove={(event) => eventHandlers.onMouseMove(event, 'horizontal')} onDoubleClick={(event) => eventHandlers.onMouseDblClick(event, 'horizontal')} onContextMenu={eventHandlers.onContextMenu}>
                <Display id="horizontal-display" type="horizontal" drawUI={true} paddingSize={10} onOutputResize={(rect) => eventHandlers.onOutputResize(rect, 'horizontal')} renderingMode={0} sourceId={sourceId}/>
              </div>)}
            {v.showVerticalDisplay && (<div className={cx(styles.studioEditorDisplayContainer, 'noselect')} style={{ cursor: v.cursor }} onMouseDown={(event) => eventHandlers.onMouseDown(event, 'vertical')} onMouseUp={(event) => eventHandlers.onMouseUp(event, 'vertical')} onMouseEnter={(event) => eventHandlers.onMouseEnter(event, 'vertical')} onMouseMove={(event) => eventHandlers.onMouseMove(event, 'vertical')} onDoubleClick={(event) => eventHandlers.onMouseDblClick(event, 'vertical')} onContextMenu={eventHandlers.onContextMenu}>
                <Display id="vertical-display" type="vertical" drawUI={true} paddingSize={10} onOutputResize={(rect) => eventHandlers.onOutputResize(rect, 'vertical')} renderingMode={0} sourceId={sourceId}/>
              </div>)}
            {v.showHorizontalDisplay && !v.showVerticalDisplay && v.studioMode && (<div id="horizontal-display-studio" className={styles.studioModeDisplayContainer}>
                <Display paddingSize={10} type="horizontal"/>
              </div>)}
            {!v.showHorizontalDisplay && v.showVerticalDisplay && v.studioMode && (<div id="vertical-display-studio" className={styles.studioModeDisplayContainer}>
                <Display paddingSize={10} type="vertical"/>
              </div>)}
          </div>
        </div>)}
      {v.isLoading && <DualOutputProgressBar sceneId={v.activeSceneId}/>}
      {!displayEnabled && (<div className={styles.noPreview}>
          {performanceMode && (<div className={styles.message}>
              {$t('Preview is disabled in performance mode')}
              <div className={cx('button button--action', styles.button)} onClick={eventHandlers.enablePreview}>
                {$t('Disable Performance Mode')}
              </div>
            </div>)}
          {!performanceMode && (<div className={styles.placeholder} ref={placeholderRef}>
              {v.studioMode && (<div className={cx(styles.placeholderControls, {
                        [styles.stacked]: studioModeStacked,
                    })}/>)}
              <img src={require('../../../media/images/16x9.png')} className={cx({
                    [styles.vertical]: verticalPlaceholder,
                    [styles.stacked]: studioModeStacked,
                    [styles.studioMode]: v.studioMode,
                })}/>
              {v.studioMode && (<img src={require('../../../media/images/16x9.png')} className={cx({ [styles.vertical]: verticalPlaceholder, [styles.stacked]: studioModeStacked }, styles.right, styles.studioMode)}/>)}
            </div>)}
        </div>)}
    </div>);
}
function StudioModeControls(p) {
    ...
    return (<div className={cx(styles.studioModeControls, { [styles.stacked]: p.stacked })}>
      <span className={styles.studioModeControl}>{$t('Edit')}</span>
      <button className="button button--default" onClick={() => TransitionsService.actions.executeStudioModeTransition()}>
        {$t('Transition')}{' '}
        {p.stacked ? (<i className="fa fa-arrow-down" v-if="stacked"/>) : (<i className="fa fa-arrow-right"/>)}
      </button>
...
function DualOutputControls(p) {
    ...
    return (<div id="dual-output-header" className={cx(styles.dualOutputHeader, { [styles.stacked]: p.stacked })}>
      {showHorizontal && (<div className={styles.horizontalHeader}>
          <i className="icon-desktop"/>
          <span>{$t('Horizontal Output')}</span>
          {showRecordingIcons && <DualOutputIcons display="horizontal"/>}
        </div>)}

      {showVertical && (<div className={styles.verticalHeader}>
...
//# sourceMappingURL=StudioEditor.jsx.map