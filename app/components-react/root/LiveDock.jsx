...
export default function LiveDockWithContext() {
    ...
    return (<LiveDockCtx.Provider value={controller}>
      <LiveDock />
...
function LiveDock() {
    ...
    const chat = useMemo(() => {
        ...
        return (<Chat restream={isRestreaming && visibleChat === 'restream'} key={visibleChat} visibleChat={visibleChat} setChat={setChat}/>);
    }, [Services.UserService.state.auth.primaryPlatform, visibleChat]);
    return (<div className={styles.liveDock}>
      <div className={styles.liveDockExpandedContents}>
        <div className={styles.liveDockHeader}>
          <div className="flex flex--center">
            <div className={cx(styles.liveDockPulse, {
        ...
        <div className={styles.liveDockInfo}>
          <div className={styles.liveDockPlatformTools}>
            {ctrl.canEditChannelInfo && (<Tooltip title={$t('Edit your stream title and description')} placement="right" autoAdjustOverflow={false}>
                <i onClick={() => ctrl.showEditStreamInfo()} className="icon-edit"/>
              </Tooltip>)}
            {hasLiveDockFeature('view-stream') && isStreaming && (<Tooltip title={$t('View your live stream in a web browser')} placement="right" autoAdjustOverflow={false}>
                <i onClick={() => ctrl.openPlatformStream()} className="icon-studio"/>
              </Tooltip>)}
            {isStreaming && <ShareStreamLink />}
            {hasLiveDockFeature('dashboard') && isStreaming && (<Tooltip title={$t('Go to Live Dashboard')} placement="right" autoAdjustOverflow={false}>
          ...
          <div className="flex">
            ...
            (hasLiveDockFeature('refresh-chat-restreaming') && isRestreaming)) && (<a onClick={() => ctrl.refreshChat()}>{$t('Refresh Chat')}</a>)}
          </div>
        </div>
        {!hideStyleBlockers &&
            (hasLiveDockFeature('chat-offline') ||
                (isStreaming && hasLiveDockFeature('chat-streaming'))) && (<div className={styles.liveDockChat}>
              {hasChatTabs && <ChatTabs visibleChat={visibleChat} setChat={setChat}/>}
              {!applicationLoading && !collapsed && chat}
              {!['default', 'restream'].includes(visibleChat) && (<PlatformAppPageView className={styles.liveDockPlatformAppWebview} appId={visibleChat} pageSlot={pageSlot} key={visibleChat}/>)}
            </div>)}
        
        {!hideStyleBlockers &&
            (!ctrl.platform || (hasLiveDockFeature('chat-streaming') && !isStreaming)) && (<div className={cx('flex flex--center flex--column', styles.liveDockChatOffline)}>
...
function ChatTabs(p) {
    ...
    return (<div className="flex">
      <Menu defaultSelectedKeys={[p.visibleChat]} onClick={ev => p.setChat(ev.key)} mode="horizontal">
        {ctrl.chatTabs.map(tab => (<Menu.Item key={tab.value}>{tab.name}</Menu.Item>))}
      </Menu>
      <div className={styles.liveDockChatTabsIcons}>
        {ctrl.isPopOutAllowed && (<Tooltip title={$t('Pop out to new window')} placement="topRight">
            <i className={cx(styles.liveDockChatTabsPopout, 'icon-pop-out-1')} onClick={() => ctrl.popOut()}/>
          </Tooltip>)}
        <Tooltip title={$t('You can now reply to Twitch, YouTube and Facebook messages in Multichat. Click to learn more.')} placement="topRight" onClick={ctrl.showMultistreamChatInfo}>
          <i className={cx(styles.liveDockChatTabsInfo, 'icon-information')}/>
        </Tooltip>
      </div>
    </div>);
}
//# sourceMappingURL=LiveDock.jsx.map