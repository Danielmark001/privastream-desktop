...
export default function TestWidgets(p) {
    ...
    return (<div className="slide-open">
      <a className="slide-open__open link" onClick={() => setSlideOpen(!slideOpen)}>
        {$t('Test Widgets')}
      </a>
      <Animation transitionName="ant-slide-right">
        {slideOpen && (<div className="slide-open__menu" style={{ zIndex: 1011 }}>
            {hasSmartBrowser &&
                VISION_TESTERS.map(tester => (<button className="button button--trans" key={`test-reactive-${tester.value}`} onClick={() => testReactive(tester.value)}>
                  {$t(tester.name)}
                </button>))}
            {widgetTesters.map(tester => (<button className="button button--trans" key={tester.name} onClick={() => test(tester.name)}>
                {$t(tester.name)}
              </button>))}
          </div>)}
      </Animation>
...
//# sourceMappingURL=TestWidgets.jsx.map