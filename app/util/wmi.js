...
export function getWmiClass(wmiClass, select) {
    try {
        const result = JSON.parse(cp
            .execSync(`Powershell -command "Get-CimInstance -ClassName ${wmiClass} | Select-Object ${select.join(', ')} | ConvertTo-JSON"`)
            .toString());
...
function convertWmiValues(wmiObject) {
